...
\begin{equation*}
    X \coloneqq \mathbf{1}_{\left\{W = 1\right\}}Y + \mathbf{1}_{\left\{W = 0\right\}}Z,
\end{equation*}
...
\begin{probox}{Basic Properties of Expectation}{basicPropsE}
    Let $X$ be a real-valued random variable, then
    \begin{enumerate}
        \item if $P\left(X \geq 0\right) = 1$, then $\mathbb{E}\left[X\right] \geq 0$;
        \item if $P\left(X = 1\right) = 1$, then $\mathbb{E}\left[X\right] = 1$;
        \item $\mathbb{E}\left[aX + b\right] = a\mathbb{E}\left[X\right] + b$ for any $a, b \in \R$;
        \item if $Y$ is another real-valued random variable such that $X$ and $Y$ are independent, then~$\mathbb{E}\left[XY\right] = \mathbb{E}\left[X\right]\mathbb{E}\left[Y\right]$.
    \end{enumerate} 
\end{probox}
...
\begin{thmbox}{Markov's Inequality}{MarkovIneq}
    ...
    \end{proof}
\end{thmbox}
Note that $\mathbb{E}\left[X\right]$ is a real number while $\mathbb{E}\left[X \mid Y\right]$ is a \textbf{random variable} as a function of~$Y$. In a way, $Y$ partitions the sample space into regions where $\mathbb{E}\left[X \mid Y = y_i\right]$ gives the expectation of $X$ in the region induced by $Y = y_i$ for each $y_i \in \mathcal{Y}$.
\begin{thmbox}{Law of Total Expectations}{totalExpectation}
...
\end{dfnbox}
Note that the variance might not be finite. Consider a continuous random variable $X$ with probability density function $f\left(x\right) = \frac{c}{1 + x^3}\mathbf{1}_{\left\{x \in \left[0, \infty\right)\right\}}$ where $c > 0$ is appropriately chosen. One can check that $\mathbb{E}\left[X\right]$ is finite but $\mathbb{E}\left[X^2\right]$ is unbounded.

Similar to our discussion of expectation, we propose the following basic property:
\begin{probox}{Basic Property of Variance}{varBasicProp}
    Let $X$ be a real-valued random variable, then 
    \begin{equation*}
...
\begin{notebox}
    \begin{remark}
        Note that $\mathrm{Cov}\left(X, Y\right) = \mathrm{Cov}\left(Y, X\right)$ and $\mathrm{Cov}\left(X, X\right) = \mathrm{Var}\left(X\right)$. If $X$ and $Y$ are independent, then $\mathrm{Cov}\left(X, Y\right) = \mathrm{Cov}\left(Y, X\right) = 0$.
    \end{remark}
\end{notebox}
Covariance can be seen as a measure for the \textbf{joint} variation of two random variables, which offers a way to assess the dependency between two random variables to some extent.
\begin{dfnbox}{Correlation}{corr}
    Let $X$ and $Y$ be random variables. The {\color{red} \textbf{correlation}} between $X$ and $Y$ is defined as 
    \begin{equation*}
        \mathrm{Corr}\left(X, Y\right) \coloneqq \frac{\mathrm{Cov}\left(X, Y\right)}{\sqrt{\mathrm{Var}\left(X\right)\mathrm{Var}\left(Y\right)}}.
    \end{equation*}
    If $\mathrm{Corr}\left(X, Y\right) = 0$, we say that $X$ and $Y$ are {\color{red} \textbf{unrelated}}.
\end{dfnbox}
Correlation is obviously symmetric. Furthermore, we have the following result:
\begin{probox}{Range of Correlation}{corrRan}
    Let $X$ and $Y$ be random variables such that their expectation and variance are both finite, then $\mathrm{Corr}\left(X, Y\right) \in \left[-1, 1\right]$. 
\end{probox}
\begin{notebox}
    \begin{remark}
        In particular, $\mathrm{Corr}\left(X, X\right) = 1$ and $\mathrm{Corr}\left(X, -X\right) = -1$.
    \end{remark}
\end{notebox}
Lastly, we propose the following result on the variance of the sum of random variables:
\begin{probox}{Variance of a Sum}{sumVar}
    Let $\left\{X_i\right\}_{i = 1}^n$ be a family of random variables such that $\mathbb{E}\left[X_i^2\right]$ is finite for all $i = 1, 2, \cdots, n$. then 
    \begin{equation*}
        \mathrm{Var}\left(\sum_{i = 1}^{n}X_i\right) = \sum_{i = 1}^{n}\mathrm{Var}\left(X_i\right) + 2\sum_{1 \leq i < j \leq n}\mathrm{Cov}\left(X_i, X_j\right).
    \end{equation*}
\end{probox}
...
\end{probox}
Analogously, another kind of generating functions concern the \textit{moments} of a random variable.
...
\begin{thmbox}{Weak Law of Large Numbers}{weakLargeN}
    Let $\left\{X_i\right\}_{i \in \N}$ be a family of independent and identically distributed random variables with mean $\mu$ and finite variance, then for any $\epsilon > 0$, 
    \begin{equation*}
        \lim_{n \to \infty}P\left(\abs{\frac{\sum_{i = 1}^{n}X_i}{n} - \mu} > \epsilon\right) = 0.
    \end{equation*}
\end{thmbox}
...
\begin{probox}{Bayes' Rule for Markov Chains}{MarkovBayes}
