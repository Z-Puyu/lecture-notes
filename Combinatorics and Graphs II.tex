...
Note that if a graph has a proper $k$-colouring, then it must also have a proper $(k + 1)$-colouring. Therefore, we are interested to know the least number of colours to properly colour a graph.
\begin{dfnbox}{Chromatic Number}{chromaticNum}
    The {\color{red} \textbf{chromatic number}} of a graph $G$, denoted by $\chi(G)$, is the smallest integer $k$ such that $G$ has a proper $k$-colouring.
\end{dfnbox}
...
\end{itemize}
Recall the Kneser graph $KG(n, k)$ from Definition \ref{dfn:kneser}.
