...
\end{equation*}
for some $\mathbfit{c} \in \R^n$. Let $f(\mathbfit{x}) = a_0 + b$, then we have $\mathbfit{c}^{\mathrm{T}}\mathbfit{x} = b$. Note that this level set equation gives a linear function in $\R^{n - 1}$, because obviously there exists some $\mathbfit{p} \in \R^{n - 1}$ such that~$\mathbfit{c}^{\mathrm{T}}\mathbfit{x} = c_nx_n + \mathbfit{p}^{\mathrm{T}}\mathbfit{x'}$, where $\mathbfit{x}' = \left(x_1, x_2, \cdots, x_{n - 1}\right)$. Therefore,
\begin{equation*}
    x_n = \frac{b}{c_n} - \frac{1}{c_n}\mathbfit{p}^{\mathrm{T}}\mathbfit{x'}.
\end{equation*} 
Apparently, $x_n$ is a linear function of $\mathbfit{x}'$. This means every straight line in $\R^n$ is defined by an equation
\begin{equation*}
...
\end{equation*}
For instance, we can take $x_j^+ = 0$ and $x_j^- > 0$ whenever $x_j < 0$ and vice versa for $x_j > 0$. Note that this correspondence is not unique.

After the above transformations, we see that every constraint is equivalent to either an equality constraint $\mathbfit{a}_i^{\mathrm{T}}\mathbfit{x} + s_i = b_i$ or an inequality constraint in the form of $x_j \geq 0$. Therefore, we define the following as the \textit{standard form} of a linear program:
\begin{align*}
    \min_{\mathbfit{x} \in \R^n} & \mathbfit{c}^{\mathrm{T}}\mathbfit{x} \\
    \textrm{s.t. } & \mathbfit{Ax = b} \\
    & x_j \geq 0, \quad \textrm{for } j = 1, 2, \cdots, n.
\end{align*}
Note that here, $\mathbfit{A}$ is an $m \times n$ matrix where the $i$-th row in $\mathbfit{A}$ contains the coefficients of the $i$-th constraint.
...
\end{equation*}
where $\mathbfit{A} \in \R^{m \times n}$. We will devise a way to compute the basic feasible solutions systematically.
\begin{thmbox}{Basic Solution Characterisation}{basicSolnChar}
    Let 
    \begin{equation*}
        P \coloneqq \left\{\mathbfit{x} \in \R^n \colon \mathbfit{Ax = b}, \mathbfit{x} \geq \mathbf{0}\right\}
    \end{equation*}
    be a polyhedron for some $\mathbfit{A} \in \R^{m \times n}$. A vector $\mathbfit{x}^* \in \R^n$ is a basic solution if and only if
    \begin{itemize}
    ...
    \begin{proof}
        ...
        \begin{equation*}
            ...
            \end{bmatrix}\mathbfit{\widebar{x}}^* = \mathbf{0},
        \end{equation*}
        where $\mathbfit{\widebar{x}}^*$ is obtained by re-arranging the rows of $\mathbfit{x}^*$ accordingly. Note that the columns of $\mathbfit{A}_B$ is linearly independent, so $\det(\mathbfit{A}_B) \neq 0$. Therefore, 
        \begin{equation*}
            \begin{vmatrix}
                \mathbfit{A}_B & \mathbfit{A}_N \\
                \mathbf{0} & \mathbfit{I}_N
...
\begin{thmbox}{Conditions for the Existence of Basic Feasible Solution}{existBFS}
    ...
    \end{proof}
\end{thmbox}
Consider the level set defined by $f(\mathbfit{x}) = \mathbfit{c}^{\mathrm{T}}\mathbfit{x} = k$ for some affine function $f$. Suppose the level set has an intersection with some polyhedron $P$, then by translating the level set in the direction of $-\nabla f$, it will eventually intersect $P$ at a basic feasible solution such that any further translation will make the intersection empty. Therefore, a reasonable guess is that an optimal solution for any linear program is closely linked to the basic feasible solutions of the feasible set.

We sketch the idea of the proof first. If $P$ is a polyhedron representing the feasible set of a linear program, it is easy to check that the set of optimal solutions, $Q \subseteq P$, is also a polyhedron. We will attempt to pick a basic feasible solution $\mathbfit{x}^*$ from $Q$ first. Then, if $\mathbfit{x}^*$ is not a basic feasible solution of $P$, we strategically choose some direction along which we can translate $\mathbfit{x}^*$ to an interior point $\mathbfit{y} \in P$ while still staying in $Q$. Once we are at $\mathbfit{y}$, we will be able to translate in the direction of $\mathbfit{c}$ by a small distance to find a point where the function evaluates to a better value than the optimal value, arriving at a contradiction.
\begin{thmbox}{Optimality of Basic Feasible Solutions}{optBFS}
    ...
    \begin{proof}
        ...
        \\\\
        Let $\mathbfit{x}^*$ be any basic feasible solution of $Q$. We claim that $\mathbfit{x}^*$ is a basic feasible solution of $P$. Suppose on contrary that $\mathbfit{x}^*$ is not a basic feasible solution of $P$. Pick some $\mathbfit{d} \in \R$ such that $\mathbfit{c}^{\mathrm{T}}\mathbfit{d} = 0$. Let $P = \left\{\mathbfit{x} \in \R^n 
        \colon \mathbfit{Ax \leq b}\right\}$. Notice that we can choose $\lambda \in \R$ such that $\mathbfit{A}\left(\mathbfit{x}^* + \lambda\mathbfit{d}\right) < \mathbfit{b}$, and so $\mathbfit{x}^* + \lambda\mathbfit{d} \in Q$ is an internal point of~$P$. However, this means that there exists some $\mu > 0$ such that 
        \begin{equation*}
            \mathbfit{x}^* - \mu\mathbfit{c} = \mathbfit{x}^* + \lambda\mathbfit{d} - \left(\mu\mathbfit{c} + \lambda\mathbfit{d}\right) \in P.
        \end{equation*}
        However, $\mathbfit{c}^{\mathrm{T}}\left(\mathbfit{x}^* - \mu\mathbfit{c}\right) < v*$, which is impossible. Therefore, $\mathbfit{x}^*$ must be a basic feasible solution of $P$.
    \end{proof}
\end{thmbox}
...
\begin{dfnbox}{Reduced Cost}{reducedCost}
    ...
    \begin{equation*}
        \widebar{c}_j = c_j - \mathbfit{c}_B^{\mathrm{T}}\mathbfit{A}_B^{-1}\mathbfit{A}_j.
    \end{equation*}
\end{dfnbox}
\begin{notebox}
    \begin{remark}
        For any $i \in B$, note that $\mathbfit{A}_i = \mathbfit{A}_B\mathbfit{e}_i$, we have $\mathbfit{A}_B^{-1}\mathbfit{A}_i = \mathbfit{e}_i$. Therefore, $\widebar{c_i} = 0$.
    \end{remark}
\end{notebox}
We say that a direction $\mathbfit{d}^j$ is an \textit{improving direction} if and only if $\widebar{c}_j < 0$. Furthermore, notice that by Theorem \ref{thm:bfsDir}, we have
\begin{align*}
    \widebar{c}_j & = c_j - \mathbfit{c}_B^{\mathrm{T}}\mathbfit{A}_B^{-1}\mathbfit{A}_j \\
...
Next, we only need to determine a $\widebar{\theta_j} > 0$ such that $\mathbfit{x} + \widebar{\theta_j}\mathbfit{d}^j$ gives us another basic feasible solution. Since we already know that $\mathbfit{A}\left(\mathbfit{x} + \widebar{\theta_j}\mathbfit{d}^j\right) = \mathbfit{b}$ for all $\widebar{\theta_j} > 0$, a natural idea is to take $\widebar{\theta_j}$ to be the greatest positive real number such that $\mathbfit{x} + \widebar{\theta_j}\mathbfit{d}^j \geq \mathbf{0}$, i.e., we will move in the direction $\mathbfit{d}^j$ until we can barely stay in the feasible polyhedron. Consider
\begin{align*}
    \mathbfit{x} + \widebar{\theta_j}\mathbfit{d}^j = \left(\mathbfit{x}_B + \widebar{\theta_j}\mathbfit{d}^j_B, \mathbfit{x}_N + \widebar{\theta_j}\mathbfit{d}^j_N\right).
\end{align*}
Note that $\mathbfit{x}_N \geq \mathbf{0}$ and $\widebar{\theta_j}\mathbfit{d}^j_N = \widebar{\theta_j}\mathbfit{e}_j \geq \mathbf{0}$, so it suffices to check that $\mathbfit{x}_B + \widebar{\theta_j}\mathbfit{d}^j_B \geq \mathbf{0}$. This implies that for each $i \in B$, we have $\widebar{\theta_j} \geq -\frac{x_i}{d^j_i}$. Therefore, we only need to take
\begin{equation*}
    \widebar{\theta_j} = \min\left\{-\frac{x_i}{d^j_i} \colon i \in B, d^j_i < 0\right\}.
\end{equation*}
Note that here we do not consider those $d^j_i$'s with $d^j_i > 0$ because in such cases $x_i + \widebar{\theta_j}d^j_i \geq 0$ for all $\widebar{\theta_j} > 0$. Now, we would verify that the new point we reach is indeed another basic feasible solution.
\begin{probox}{Criterion for Adjacent Basic Feasible Solutions}{anotherBFS}
    If $\left\{i \in B \colon d_i^j < 0\right\} \neq \varnothing$ and $\mathbfit{x}$ is a basic feasible solution, then $\mathbfit{x} + \widebar{\theta_j}\mathbfit{d}^j$ is a basic feasible solution adjacent to $\mathbfit{x}$.
    \tcblower
    \begin{proof}
        Note that $\mathbfit{x} + \widebar{\theta_j}\mathbfit{d}^j$ is feasible, so $\mathbfit{A}\left(\mathbfit{x} + \widebar{\theta_j}\mathbfit{d}^j\right) = \mathbfit{b}$. Note that by the definition of $\widebar{\theta_j}$, we have 
        \begin{equation*}
            \widebar{\theta_j} = -\frac{x_{\ell}}{d_{\ell}^j}
        \end{equation*}
        for some $\ell \in B$. Therefore, we have $\left(\mathbfit{x} + \widebar{\theta_j}\mathbfit{d}^j\right)_{\ell} = 0$. Notice that at $\mathbfit{x} \coloneqq \left(\mathbfit{x}_B, \mathbfit{x}_N\right)$, we have~$\ell \in B$ and $j \in N$. Consider $\widebar{B} \coloneqq (B - \{\ell\}) \cup \{j\}$ and $\widebar{N} \coloneqq (N - \{j\}) \cup \{\ell\}$. One may check that $\widebar{B}$ is linearly independent, so by Theorem \ref{thm:basicSolnChar}, $\mathbfit{x} + \widebar{\theta_j}\mathbfit{d}^j$ is a basic feasible solution.
    \end{proof}
\end{probox}
...
\begin{notebox}
    ...
    \end{remark}
\end{notebox}
Recall that along an improving direction $\mathbfit{d}^j$, the reduced cost for $x_j$ is $\widebar{c}_j < 0$. A natural observation here is that if we cannot find any improving direction at a point $\mathbfit{x}^*$, then this point must be optimal. Therefore, we are tempted to conclude that $\widebar{c}_j \geq 0$ for all $j \in N$.
\begin{thmbox}{Optimality Conditions for Simplex Method}{simplexOptCond}
    Let $\mathbfit{x}^* = \left(\mathbfit{x}_B, \mathbfit{x}_N\right)$ be a basic feasible solution to some standard linear program. Let $\mathbfit{\widebar{c}}$ be the vector of reduced costs associated with $\mathbfit{x}^*$, then 
    \begin{enumerate}
        \item If $\mathbfit{\widebar{c}} \geq \mathbf{0}$, then $\mathbfit{x}^*$ is optimal;
        \item If $\mathbfit{x}^*$ is optimal and non-degenerate, then $\mathbfit{\widebar{c}} \geq \mathbf{0}$.
    \end{enumerate}
    \tcblower
    \begin{proof}
        Suppose that $\mathbfit{\widebar{c}} \geq \mathbf{0}$. Let $\mathbfit{y}$ be any feasible solution, then $\mathbfit{y - x}^*$ is a feasible direction. By Proposition \ref{pro:fDirLinCombi}, 
        \begin{equation*}
        ...
        \begin{align*}
            ...
            & = \mathbfit{c}^{\mathrm{T}}\mathbfit{x}^* + \sum_{j \in N}\lambda_j\widebar{c}_j \\
            & \geq \mathbfit{c}^{\mathrm{T}}\mathbfit{x}^*.
        \end{align*}
        Therefore, $\mathbfit{x}^*$ is an optimal solution.
        \\\\
        Suppose that $\mathbfit{x}^*$ is a non-degenerate optimal solution. For each $j \in N$, consider the adjacent basic feasible solution $\mathbfit{x} + \widebar{\theta_j}\mathbfit{d}^j$. Notice that 
        \begin{equation*}
            \mathbfit{c}^{\mathrm{T}}\mathbfit{x} \leq \mathbfit{c}^{\mathrm{T}}\left(\mathbfit{x} + \widebar{\theta_j}\mathbfit{d}^j\right) = \mathbfit{c}^{\mathrm{T}}\mathbfit{x} + \widebar{\theta_j}\widebar{c}_j
        \end{equation*}
        for all $j \in N$. Therefore, we must have $\widebar{\theta_j}\widebar{c}_j \geq 0$ for all $j \in N$. Since $\mathbfit{x}$ is non-degenerate, for each $j \in N$ we have 
        \begin{equation*}
            \widebar{\theta_j} = -\frac{x_{\ell}}{d^j_{\ell}} > 0,
        \end{equation*}
        and so $\widebar{c}_j \geq 0$. Note that for all $i \in B$, we have $\widebar{c_i} = 0$, so $\mathbfit{\widebar{c}} \geq \mathbf{0}$.
    \end{proof}
\end{thmbox}
Recall that $\mathbfit{\widebar{c}}_B = \mathbf{0}$, so it suffices to check that $\mathbfit{\widebar{c}}_N \geq \mathbf{0}$ for the $(n - m)$ inequality constraints to determine whether $\mathbfit{x}^*$ is an optimal solution.
...
\begin{tecbox}{Simplex Method}{simplex}
    ...
    \begin{enumerate}
        ...
        \begin{equation*}
            \widebar{c}_j = c_j - \mathbfit{c}_{B_k}^{\mathrm{T}}\mathbfit{A}_{B_k}^{-1}\mathbfit{A}_j.
        \end{equation*}
        \item If $\widebar{c}_j \geq 0$ for all $j \in N_k$:
        \begin{itemize}
            \item $\mathbfit{x}_k$ is an optimal solution.
        \end{itemize}
        \item Otherwise:
        \begin{enumerate}
            \item Take some $j \in N_k$ such that $\widebar{c}_j < 0$. $\left(\mathbfit{x}_{k}\right)_j$ is called an {\color{red} \textbf{entering variable}}.
            ...
            \item Otherwise:
            \begin{enumerate}
                \item Let $\ell \in B_k$ be such that $\widebar{\theta_j} = \min\left\{-\frac{x_i}{d^j_i} \colon i \in B, d^j_i < 0\right\} = \frac{x_{\ell}}{d^j_{\ell}}$. $\left(\mathbfit{x}_{k}\right)_{\ell}$ is called a {\color{red} \textbf{leaving variable}}.
                \item Update $B_{k + 1} \coloneqq (B - \{\ell\}) \cup \{j\}$.
                \item Update $\mathbfit{x}_{k + 1}$ by 
                \begin{equation*}
                    \left(\mathbfit{x}_{k + 1}\right)_i = \begin{cases}
                        \left(\mathbfit{x}_{k}\right)_i + \widebar{\theta_j}\mathbfit{d}^j_i & \quad\textrm{if } i \in B - \{\ell\} \\
                        \widebar{\theta_j} & \quad\textrm{if } i = j \\
...
\begin{center}
    \begin{tabular}{|c|c|c|}
        ...
        \hline
        $\mathbfit{\widebar{c}}$ & $\mathbfit{c}^{T} - \mathbfit{c}_B^{\mathrm{T}}\mathbfit{A}_B^{-1}\mathbfit{A}$ & $-\mathbfit{c}_B^{\mathrm{T}}\mathbfit{A}_B^{-1}\mathbfit{b}$ \\
        \hline
...
\begin{center}
    \begin{tabular}{|c|ccc|c|}
        ...
        \hline
        $\mathbfit{\widebar{c}}$ & $\widebar{c_1}$ & $\cdots$ & $\widebar{c}_n$ & $-\mathbfit{c}^{\mathrm{T}}\mathbfit{x}_B$ \\
        \hline
        $x_{B(1)}$ & & & & \\
        $\vdots$ & & & & \\
...
\begin{center}
    \begin{tabular}{|c|cc|c|}
        ...
        \hline
        $\mathbfit{\widebar{c}}$ & $\mathbf{0}$ & $\mathbfit{c}_N^{T} - \mathbfit{c}_B^{\mathrm{T}}\mathbfit{A}_B^{-1}\mathbfit{A}_N$ & $-\mathbfit{c}_B^{\mathrm{T}}\mathbfit{A}_B^{-1}\mathbfit{b}$ \\
        \hline
...
\begin{equation*}
    ...
    \end{bmatrix}
\end{equation*}
such that the $j$-th column becomes $\mathbfit{e}_{i}$. Then, we will swap the row label $\mathbfit{x}_{\ell}$ with the column label $\mathbfit{x}_j$. We repeat this process until $\mathbfit{\widebar{c}} \geq \mathbf{0}$, where we will extract $\mathbfit{A}_B^{-1}\mathbfit{b}$ from the table as our $\mathbfit{x}_B$.
...
\begin{equation*}
    \widebar{\theta_j} = \min\left\{-\frac{x_i}{d^j_i} \colon i \in B, d^j_i < 0\right\} = -\frac{x_{\ell_1}}{d^j_{\ell_1}} = -\frac{x_{\ell_2}}{d^j_{\ell_2}}.
\end{equation*}
...
\begin{equation*}
    x_{\ell_2}' = x_{\ell_2} + \widebar{\theta_j}d^j_{\ell_2} = x_{\ell_2} - \frac{x_{\ell_1}}{d^j_{\ell_1}}d^j_{\ell_2} = x_{\ell_2} - \frac{x_{\ell_2}}{d^j_{\ell_2}}d^j_{\ell_2} = 0,
\end{equation*}
so $x_{\ell_2}$ is indeed degenerate by Definition \ref{dfn:standardDegeneracy}.

Note also that in some cases, we may have more than one optimal solution for the linear program. Intuitively, this occurs when at an optimal basic feasible solution, we can still find some feasible direction along which the objective value does not deteriorate. Therefore, \textbf{an alternative optimal solution can be found if and only if some reduced cost} $\widebar{c}_j = 0$ at an optimal solution. There are $2$ specific cases:
\begin{enumerate}
    ...
    \end{equation*}
    \item If at an optimal solution $\mathbfit{x}^*$, there is $\widebar{c}_j = 0$ but $\mathbfit{d}^j \geq \mathbf{0}$, it means that $\mathbfit{x}^* + \theta\mathbfit{d}^j \geq \mathbf{0}$ is feasible for any $\theta \geq 0$, so the set of optimal solutions given by 
    \begin{equation*}
        \left\{\mathbfit{x}^* + \theta\mathbfit{d}^j \colon \theta \geq 0\right\}
    \end{equation*}
    is unbounded.
\end{enumerate}
Similar to case $2$ above, if at any basic feasible solution $\mathbfit{x}^*$, there is some $j \in N$ such that $\widebar{c}_j < 0$ and $\mathbfit{d}^j \geq \mathbf{0}$, then any $\mathbfit{x}^* + \theta\mathbfit{d}^j$ where $\theta > 0$ is feasible and improves the objective value. This implies that the optimal value can be \textbf{improved indefinitely} and so the program is unbounded.
...
\begin{thmbox}{Strong Duality}{strongDuality}
    ...
    \begin{proof}
        ...
        \begin{equation*}
            \widebar{\mathbfit{c}}^{\mathrm{T}} = \mathbfit{c}^{\mathrm{T}} - \mathbfit{c}_B^{\mathrm{T}}\mathbfit{A}_B^{-1}\mathbfit{A} \geq \mathbf{0}.
        \end{equation*}
...
\begin{equation*}
    \widebar{\mathbfit{c}} = \mathbfit{c}^{T} - \mathbfit{c}_B^{\mathrm{T}}\mathbfit{A}_B^{-1}\mathbfit{A} \geq \mathbf{0}.
\end{equation*}
This gives the implementation for the \textit{primal algorithm}, which is exactly Technique \ref{tec:simplex}.

Conversely, by maintaining $\widebar{\mathbfit{c}} \geq \mathbf{0}$, we only need to achieve $\mathbfit{A}_B^{-1}\mathbfit{b} \geq \mathbf{0}$ to make $\mathbfit{x}$ feasible, because $\mathbfit{Ax} = \mathbfit{b}$ always holds. This leads to the \textit{dual simplex method}.
\begin{tecbox}{Dual Simplex Method}{dualSimplex}
    ...
    \begin{enumerate}
        \item Manipulate the constraints and add slack variables so that the right-most portion of $\mathbfit{A}$ becomes an identity matrix.
        \item Run simplex method on the transformed problem, while maintaining $\widebar{\mathbfit{c}} \geq \mathbf{0}$.
        \begin{enumerate}
            ...
            \begin{enumerate}
                ...
                \item Otherwise, take 
                \begin{equation*}
                    i = \argmin_{j \in N_k}\left\{\frac{\bar{c}_j}{\abs{d^j_{\ell}}} \colon d^j_{\ell} < 0\right\}
                \end{equation*}
...
\begin{equation*}
    \widebar{c}_j' = c_j + \delta - \mathbfit{c}_B^{\mathrm{T}}\mathbfit{A}_B^{-1}\mathbfit{A} = \widebar{c}_j + \delta.
\end{equation*}
Clearly, optimality remains if and only if $\delta \geq -\widebar{c}_j$. If $x_j$ is basic, then for each $i \in N$, we need 
\begin{align*}
    ...
    & = \widebar{c}_j - \delta\mathbfit{e}_j^{\mathrm{T}}\mathbfit{A}_B^{-1}\mathbfit{A}_i \\
    & \geq 0.
\end{align*}
If $\mathbfit{e}_j^{\mathrm{T}}\mathbfit{A}_B^{-1}\mathbfit{A}_i > 0$, we have $\delta \leq \frac{\widebar{c}_j}{\mathbfit{e}_j^{\mathrm{T}}\mathbfit{A}_B^{-1}\mathbfit{A}_i}$. If $\mathbfit{e}_j^{\mathrm{T}}\mathbfit{A}_B^{-1}\mathbfit{A}_i < 0$, we have $\delta \geq \frac{\widebar{c}_j}{\mathbfit{e}_j^{\mathrm{T}}\mathbfit{A}_B^{-1}\mathbfit{A}_i}$. If $\mathbfit{e}_j^{\mathrm{T}}\mathbfit{A}_B^{-1}\mathbfit{A}_i = 0$, it is clear that $\widebar{c}_j$ is unchanged. Therefore, let $\widebar{a}_{i, j} = \mathbfit{e}_j^{\mathrm{T}}\mathbfit{A}_B^{-1}\mathbfit{A}_i$, we have
\begin{equation*}
    \max_{\widebar{a}_{i, j} < 0} \frac{\widebar{c}_j}{\widebar{a}_{i, j}} \leq \delta \leq \min_{\widebar{a}_{i, j} > 0} \frac{\widebar{c}_j}{\widebar{a}_{i, j}}.
\end{equation*}
...
\begin{align*}
    \widebar{c}_j' & = c_j - \mathbfit{c}_B^{\mathrm{T}}\mathbfit{A}_B^{-1}\left(\mathbfit{A}_j + \delta\mathbfit{e}_i\right) \\
    & = \widebar{c}_j - \delta\mathbfit{c}_B^{\mathrm{T}}\mathbfit{A}_B^{-1}\mathbfit{e}_i \\
    & = \widebar{c}_j - \delta p_i.
\end{align*}
Clearly, if $\widebar{c}_j - \delta p_i \geq 0$, the original optimal solution remains optimal in the altered problem, so we obtain a condition on $\delta$ to maintain the optimality.
...
\begin{equation*}
    \widebar{c}_{n + 1} = c_{n + 1} - \mathbfit{c}_B^{\mathrm{T}}\mathbfit{A}_B^{-1}\mathbfit{A}_{n + 1}.
\end{equation*}
If $\widebar{c}_{n + 1} \geq 0$, this new basic feasible solution is still an optimal solution. Otherwise, this implies that $x_{n + 1}$ can be an entering variable, and we should use the simplex method to find the optimal solution for the update problem.
...
\begin{probox}{Spanning Tree Induces Basis on $\tilde{\mathbfit{A}}$}{spanBasis}
    Let $\mathbfit{A}$ be the node-arc incidence matrix for a network $G$. An index set $B$ is a basis of $\widetilde{A}$ if and only if the set $E_B \coloneqq \left\{e_i \in E(G) \colon i \in B\right\}$ induces a spanning tree of $G$.
    \tcblower
    \begin{proof}
        Without loss of generality, suppose $\widetilde{\mathbfit{A}}$ is the first $(n - 1)$ rows of $\mathbfit{A}$. Suppose $E_B$ induces a spanning tree of $G$, then it suffices to prove that $\widetilde{\mathbfit{A}}_B$ is invertible. We shall prove this by showing that the rows of $\widetilde{\mathbfit{A}}$ are linearly independent. Suppose $\mathbfit{w} \in \R^{n - 1}$ is such that $\mathbfit{w}^{\mathrm{T}}\widetilde{\mathbfit{A}}_B = \mathbf{0}$, then $\mathbfit{w}^{\mathrm{T}}\widetilde{\mathbfit{A}}_i = 0$ for all $i \in B$. Let $e_i = (v_k, v_h)$. Observe that if $v_n \notin e_i$, then $w_k = w_h$. If $v_k = v_n$, then $w_h = 0$ and vice versa. Since $E_B$ induces a spanning tree $T$, there is some $j \in B$ such that $e_j$ is incident to $v_n$, and so there is some $w_r = 0$. Note that for all neighbours $v_m$ connected to $v_r$ in $T$, $w_m = 0$. This means $\mathbfit{w} = 0$.
    \end{proof}
\end{probox}
Now, we consider the reduced cost. Recall that $\mathbfit{p}$ such that $\mathbfit{p}^{\mathrm{T}} = \mathbfit{c}^{\mathrm{T}}_B\widetilde{\mathbfit{A}}_B^{-1}$ is an optimal solution to the dual problem. Therefore, the reduced costs are 
\begin{equation*}
    \widebar{\mathbfit{c}}^{\mathrm{T}} = \mathbfit{c}^{\mathrm{T}} - \mathbfit{p}^{\mathrm{T}}\widetilde{\mathbfit{A}}.
\end{equation*}
Since the $n$-th row of $\mathbfit{A}$ is removed in the truncated matrix, we have 
\begin{equation*}
    \widebar{c}_{(i, j)} = \begin{cases}
        c_{(i, j)} - (p_i - p_j) & \textrm{if } i, j \neq n \\
        c_{(i, j)} - p_i & \textrm{if } j = n \\
        c_{(i, j)} + p_j & \textrm{if } i = n
    \end{cases}.
\end{equation*}
By defining $p_n = 0$, we obtain $\widebar{c}_{(i, j)} = c_{(i, j)} - (p_i - p_j)$ for all $(i, j) \in E(G)$. Let $B$ be a basis inducing a spanning tree $T$ on the network $G$. Note that if $(i, j) \in B$, we must have $\widebar{c}_{(i, j)} = 0$, so we have 
\begin{align*}
    p_i - p_j & = c_{(i, j)} \quad \textrm{for all } (v_i, v_j) \in E(T), \\
    p_n = 0.
\end{align*}
Note that $V(T) = V(G)$, so solving the above system of equations gives the values of the $p_i$'s for all $i = 1, 2, \cdots, n$, which then allows us to compute $\widebar{\mathbfit{c}}$.
\\\\
Suppose we take a basis $B$ corresponding to the spanning tree $T$ of a network $G$. If there is some $(i, j) \notin B$ such that $\widebar{c}_{(i, j)} < 0$, it means the edge $(v_i, v_j)$ should be added to improve the flow. Note that there is an undirected $v_i$-$v_j$ path in $T$, so $T + (v_i, v_j)$ contains a cycle $C$ containing $v_i$ and $v_j$. We define the direction of $(v_i, v_j)$ as the forward direction and let $C_f$ and $C_b$ be the sets of forward and backward edges in $C$ respectively. 
\\\\
To improve the cost, we need to increase the flow in $(v_i, v_j)$ as much as possible. Suppose we set $x_{(i, j)} = \theta^*$, then the amount of forward flow in every $C_f \in C_f$ increases by $\theta^*$, which means we need to reduce the backward flow in every $e_b \in C_b$ by $\theta^*$ as well. Intuitively, we can take 
\begin{equation*}
...
\begin{tecbox}{Network Simplex Method}{networkSimplex}
    ...
    \begin{enumerate}
        \item Take a spanning tree $T_0 \subseteq G$ and find a basis $B$ from $E(T_0)$ and a feasible tree solution $\mathbfit{x}_0$.
        \item At the $k$-th iteration, compute the dual vector $\mathbfit{p}_k^{\mathrm{T}} = \mathbfit{c}^{\mathrm{T}}_{B_k}\widetilde{\mathbfit{A}}_{B_k}^{-1}$. Define $p_{n_{k}} = 0$. 
        \item For each edge $u_iu_j \in E(G)$, compute the reduced cost $\widebar{c}_{(i, j)} = c_{(i, j)} - (p_{i_k} - p_{j_k})$.
        \item If $\widebar{c}_{(i, j)} \geq 0$ for all $(i, j) \in E(G)$, then $\mathbfit{x}_k$ is optimal. 
        \item Otherwise, choose $(i, j) \notin B$ with $\widebar{c}_{(i, j)} < 0$.
        \item Let $C_k$ be the cycle contained in $T + u_iu_j$ and $C_f$ and $C_b$ be the forward and backward edges in $C_k$ respectively, where $u_iu_j$ induces the forward direction. 
        \begin{enumerate}
            \item If $C_b = \varnothing$, then the optimal cost is unbounded.
            \item Otherwise, compute
            \begin{equation*}
                \theta^* \coloneqq x_{pq} = \min_{(u_k, u_{\ell}) \in C_b}x_{k\ell}.
            \end{equation*}
        \end{enumerate}
        \item Update $\mathbfit{x}_k$ to $\mathbfit{x}_{k + 1}$ by 
        \begin{equation*}
            \yhwidehat{x_{k\ell}} = \begin{cases}
                x_{k\ell} + \theta^* & \textrm{if } u_ku_{\ell} \in C_f \\
                x_{k\ell} - \theta^* & \textrm{if } u_ku_{\ell} \in C_b \\
                x_{k\ell} & \textrm{otherwise}
            \end{cases}.
        \end{equation*}
        \item Update $T_{k + 1} = \left(T - u_pu_q\right)\cup u_iu_j$.
    \end{enumerate}
\end{tecbox}
