...
\end{equation*}
for some $\mathbfit{c} \in \R^n$. Let $f(\mathbfit{x}) = a_0 + b$, then we have $\mathbfit{c}^{\mathrm{T}}\mathbfit{x} = b$. Note that this level set equation gives a linear function in $\R^{n - 1}$, because obviously $\mathbfit{c}^{\mathrm{T}}\mathbfit{x} = c_nx_n + \mathbfit{\widebar{c}}^{\mathrm{T}}\mathbfit{x'}$, so
\begin{equation*}
    x_n = \frac{b}{c_n} - \frac{1}{c_n}\mathbfit{\widebar{c}}^{\mathrm{T}}\mathbfit{x'}.
\end{equation*} 
Apparently, $-\frac{1}{c_n}\mathbfit{\widebar{c}} \in \R^{n - 1}$, so $x_n$ is a linear function of $\mathbfit{x}'$. This means every straight line in $\R^n$ is defined by an equation
\begin{equation*}
...
\begin{probox}{$\mathbfit{x} + \bar{\theta_j}\mathbfit{d}^j$ Is A Basic Feasible Solution}{anotherBFS}
    If $\left\{i \in B \colon d_i^j < 0\right\} \neq \varnothing$, then $\mathbfit{x} + \widebar{\theta_j}\mathbfit{d}^j$ is a basic feasible solution adjacent to $\mathbfit{x}$.
    \tcblower
    \begin{proof}
