...
Corollary \ref{cor:condDoesNotIncrEntropy} can be generalised further. We first introduce a preliminary definition.
...
\begin{corbox}{Generalised Inequality for Conditional Entropy}{genIneqCondEntropy}
...
\begin{probox}{Convexity of Relative Entropy}{convextRelativeEntropy}
    ...
    \tcblower
    \begin{proof}
        Let $p_1, p_2, q_1, q_2$ be probability mass functions for the same random variable $X$. It suffices to prove that 
        \begin{equation*}
...
\end{thmbox}
\begin{thmbox}{Data Processing Inequality}{DPI}
    ...
    \begin{proof}
        ...
        \end{equation*}
        Since $X$-$Y$-$Z$ forms a Markov chain, $I\left(X ; Z \mid Y\right) = 0$.
    \end{proof}
\end{thmbox}
\begin{corbox}{Mutual Information Does Not Increase After Processing}{nonIncrI}
    For any random variables $X$ and $Y$, we have $I\left(X ; Y\right) \geq I\left(X ; g\left(Y\right)\right)$ for all function $g$.
\end{corbox}
...
\end{dfnbox}
Let $X$ be any random variable and $Y$ be another random variable correlated to $Y$. Suppose now we wish to estimate $X$ via observations about $Y$. Let $\hat{X}\left(Y\right)$ be an estimator obtained this way about $X$.
...
\begin{thmbox}{Fano's Inequality}{Fano}
    For any estimator $\hat{X}$, $H_b\left(p_e\right) + p_e\log_2\abs{X} \geq H\left(X \mid \hat{X}\right) \geq H\left(X \mid Y\right)$.
    \tcblower
    \begin{proof}
        Define $E \coloneqq \mathbf{1}\left\{\hat{X} \neq X\right\}$ to be the error random variable. Consider 
        \begin{equation*}
            H\left(E \mid \hat{X}\right) + H\left(X \mid E, \hat{X}\right) = H\left(E, X \mid \hat{X}\right) = H\left(X \mid \hat{X}\right) + H\left(E \mid X, \hat{X}\right).
        \end{equation*}
    \end{proof}
\end{thmbox}
\end{document}