...
\begin{notebox}
    \begin{remark}
        For any set $X$, we define $\{\varnothing, X\}$ as the \textit{trivial topology} on $X$, $\mathcal{P}(X)$ as the \textit{discrete topology}, and $\left\{X \backslash U \colon U \subseteq X \textrm{ is finite}\right\} \cup \{\varnothing\}$ as the \textit{co-finite topology}.
    \end{remark}
\end{notebox}
