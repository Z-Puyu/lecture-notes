...
\begin{dfnbox}{Partial Order and Total Order}{order}
    ...
    \end{itemize}
    A {\color{red} \textbf{total order}} on a set $X$ is a partial order on $X$ such that $x \leq y$ or $y \leq x$ for any $x, y \in X$.
\end{dfnbox}
...
\begin{probox}{Characterisation of the Maximal Containing Collection}{charMaximal}
    Let $X$ be a set and $\mathcal{A} \subseteq \mathcal{P}\left(X\right)$ be a collection with the finite intersection property. For each maximal collection $\mathcal{D} \subseteq \mathcal{P}\left(X\right)$ with $\mathcal{A} \subseteq \mathcal{D}$ with the finite intersection property,~$\mathcal{D}$ is closed under finite intersections and for every $A \subseteq X$, if $A \cap D \neq \varnothing$ for all $D \in \mathcal{D}$, then $A \in \mathcal{D}$.
\end{probox}
...
\begin{thmbox}{Tychonoff's Theorem}{Tychonoff}
    If $\left\{X_{\alpha}\right\}_{\alpha \in \Lambda}$ is a family of compact topological spaces, then $\prod_{\alpha \in \Lambda}X_{\alpha}$ is compact.
\end{thmbox}
...
\begin{probox}{Sequentially Compact Spaces Guarantee Lebesgue Number}{seqCompactLeb}
    ...
    \tcblower
    \begin{proof}
        Let $X$ be sequentially compact. Suppose on contrary that there exists some open cover $\mathcal{U}$ of $X$ without a Lebesgue number, then for all $n \in \N^+$, there exists some $S_n \subseteq X$ such that $\mathrm{diam}\left(S_n\right) < \frac{1}{n}$. For each $n \in \N^+$, pick some $x_n \in S_n$ to form a sequence, then this sequence has a convergent subsequence $\left\{x_{n_i}\right\}_{i \in \N^+}$. Let $\lim_{i \to \infty}x_{n_i} = x$ and $x \in U$ for some $U \in \mathcal{U}$, then there exists some $\epsilon > 0$ such that $B_{\epsilon}\left(x\right) \subseteq U$. Note that there exists some $N \in \N^+$ such that $\frac{1}{N} < \frac{\epsilon}{2}$ and $x_{n_i} \in B_{\frac{\epsilon}{2}}\left(x\right)$ for all $i > N$. Note that for all $i > N$, we have $n_i \geq i > N$, so 
        \begin{equation*}
...
\begin{probox}{Compactness of Metrisable Spaces}{metricCompact}
    ...
    \begin{proof}
        ...
        \end{equation*}
        We claim that $B_i \cap \left(A \setminus \left\{x\right\}\right)$ is infinite. Suppose on contrary that it is finite,
    \end{proof}
\end{probox} 
...
\begin{probox}{Properties of Total Boundedness}{propsTotalBound}
    \begin{enumerate}
        \item If $\left(X, d\right)$ is a metric space and $\rho\left(x, y\right) = \frac{d\left(x, y\right)}{1 + d\left(x, y\right)}$, then $\left(X, \rho\right)$ is totally bounded if and only if $\left(X, d\right)$ is totally bounded.
        \item If $\left(X, d\right)$ and $\left(X', d'\right)$ are bi-Lipschitz, i.e., there exists some $A > 1$ such that 
        \begin{equation*}
            \frac{1}{A}d\left(x, y\right) < d'\left(x, y\right) < Ad\left(x, y\right),
        \end{equation*}
        then $\left(X, d\right)$ is totally bounded if and only if $\left(X', d'\right)$ is totally bounded.
        \item Every subspace $X$ of $\left(\R^n, \ell_p\right)$ is totally bounded if and only if $X$ is bounded.
    \end{enumerate}
\end{probox}
