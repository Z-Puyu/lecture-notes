...
Note that a topological space can be completely captured by its basis, so an important property of a topological space is the countability of its basis. In fact, this has been demonstrated in the previous section where a regular space becomes normal if it has a countable basis. Roughly speaking, the countability of the basis represents how ``easy'' it is to generate all open sets in the space using some fixed open sets.
...
\end{dfnbox}
We introduce the following interesting result for second countable spaces:
\begin{probox}{Dense Countable Subspaces in Second Countable Spaces}{denseCountable}
    If $X$ is a second countable space, then there exists a countable subset $A \subseteq X$ such that $A$ is dense in $X$.
\end{probox}
...
\begin{probox}{Continuous Map Characterisation of First Countable Spaces}{characterise1stCountableContinuous}
    ...
    \end{proof}
\end{probox}
Lastly, we shall prove that both subspaces and product spaces preserve first and second countability.
\begin{probox}{Cartesian Product and Subspace Preserve Countability}{cartesianProductSubspaceCountability}
    If $X$ is first or second countable, then every subset $A \subseteq X$ is first or second countable respectively. Furthermore, if $Y$ is also first or second countable respectively, then $X \times Y$ is first or second countable respectively.
\end{probox}
