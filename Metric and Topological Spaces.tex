...
\begin{dfnbox}{Box Topology}{box}
    Let $\left\{\left(X_{\alpha}, \mathcal{T}_{\alpha}\right)\right\}_{\alpha \in \Lambda}$ be a sequence of non-empty topological spaces. The {\color{red} \textbf{box topology}} is the topology generated by the sub-basis 
    \begin{equation*}
...
\end{probox}
Let $f \colon \R \to \R^{\N}$ such that $f\left(x\right) = \left(x, x, \cdots\right)$. We claim that $f$ is not continuous with respect to the box topology.
\begin{corbox}{}{}
...
\end{corbox}
\begin{probox}{}{}
    Let $\left(X, \mathcal{T}_X\right)$ and $\left(Y, \mathcal{T}_Y\right)$ be topological spaces. Let $A \subseteq X$, $B \subseteq Y$ and $\mathcal{T}_{A \times B}$ be the subspace topology on $A \times B$ induced by $\mathcal{T}_X \times \mathcal{T}_Y$, then $\mathcal{T}_{A \times B} = \mathcal{T}_A \times \mathcal{T}_B$.
\end{probox}
\begin{probox}{}{}
    Let $n \in \Z^+$ and $n = \sum_{i = 1}^km_i$ with $m_i \in \Z^+$ for $i = 1, 2, \cdots, k$, then 
    \begin{equation*}
        \prod_{i = 1}^k\R^{m_i} = \R^n.
    \end{equation*}
\end{probox}
