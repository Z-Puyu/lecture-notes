...
\end{dfnbox}
We shall prove that the co-finite topology is always $T_1$.
...
Note that $T_1$ spaces are essentially spaces in which for any two different points, we can find an open set to ``separate'' them by secluding the first point with the open set. However, notice that the second point might not be contained by an open set in the set-up, which can be strengthened.
