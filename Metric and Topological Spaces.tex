...
\begin{thmbox}{Embedding Theorem}{embedding}
    ...
    \tcblower
    \begin{proof}
        By Proposition \ref{pro:continuousProj}, $F$ is continuous because all of its component maps, the $f_{\alpha}$'s, are continuous.
    \end{proof}
\end{thmbox}
...
Therefore, the following result is easy to show:
...
\end{probox}
We shall prove that a finite co-finite topological space is always $T_1$.
\begin{probox}{Co-finite Topology Is $T_1$}{T1Cofinite}
    ...
    \begin{proof}
        ...
        \\\\
        We shall also prove that if $X$ is infinite, then $X$ is not Hausdorff. Take any~$x \in X$ and $U \in \mathcal{T}$ with $x \in U$, then $X \setminus U$ is finite. For all $y \in X$ with $y \neq x$, we have $y \in X \setminus U$. However, for all $V \subseteq X \setminus U$, we have $V \notin \mathcal{T}$ because $X \setminus V$ is infinite. Therefore, $X$ is not Hausdorff.
    \end{proof}
\end{probox}
...
\end{dfnbox}
Notice that every normal space is $T_1$. In fact, a normal space is regular, a regular space is Hausdorff, and a Hausdorff space is $T_1$. We can characterise a normal space using the following result:
\begin{probox}{Characterisation of Normal Spaces}{normalSpace}
    ...
    \tcblower
    \begin{proof}
        Suppose that $X$ is normal. Take any closed subset $A \subseteq X$ and any open set $U$ with $A \subseteq U$. Note that $B \coloneqq X \setminus U$ is closed and $A \cap B = \varnothing$, so there exists disjoint open sets $W_A, W_B \subseteq X$ such that $A \subseteq W_A$ and $B \subseteq W_B$. Take $W \coloneqq X \setminus W_B \subseteq X \setminus B = U$, which is closed, then 
        \begin{equation*}
            W_A \subseteq X \setminus W_B = W \subseteq U.
        \end{equation*}
        Since $W_A$ is open, this means that $W_A \subseteq \mathring{W}$. Take $V \coloneqq \mathring{W}$, then 
        \begin{equation*}
            A \subseteq V \subseteq \widebar{V} = W \subseteq U.
        \end{equation*}
...
\begin{probox}{Regular Spaces with Countable Bases Are Normal}{regWithCountableBasis}
    A regular space with a countable basis is normal.
\end{probox}
\section{Separation by Continuous Functions}
Recall that in Proposition \ref{pro:metricSpaceIsNormal}, we proved that every metrisable space is normal. This in turn demonstrates how metrisability is a very good property in topological spaces. In this chapter, we discuss various characterisations and properties of metrisable spaces.
\begin{thmbox}{Urysohn's Metrisation Theorem}{urysohnMetrisation}
    Every regular topological space with a countable basis is metrisable.
\end{thmbox}
Next, we make use of continuous functions to define an alternative notion of separability.
...
\end{dfnbox}
We see that separation by continuous functions is a stronger notion of separability as it offers a quantitative description for disjointness. With this, we can define the following characterisations:
...
\end{dfnbox}
Since separability by continuous functions is a stronger condition, we expect the following to be true:
...
\end{probox}
Now, a truly marvellous aspect of normal spaces is that their normality is ``always complete''.
