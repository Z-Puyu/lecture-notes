...
\begin{equation*}
    A \coloneqq \left\{\frac{1}{n} \colon n \in \N^+\right\}
\end{equation*}
again. It is easy to verify that $0$ is the only limit point of $A$, so by Proposition \ref{clProps},
\begin{equation*}
    \widebar{A} = \left\{0\right\} \cup \left\{\frac{1}{n} \colon n \in \N^+\right\} \neq A.
\end{equation*}
Therefore, $A$ is neither open nor closed in $\R$. This also shows that $\partial A \neq A'$ in general.
...
\begin{dfnbox}{Supremum Metric}{supMetric}
    ...
    \end{equation*}
\end{dfnbox}
A basic yet important mapping between two spaces is the \textit{identity map}. In a metric space, however, we can further generalise the identity map into maps which preserve the distance between points.
...
\end{dfnbox}
Recall that with compactification, we can construct a compact Hausdorff space from a general topological space. Notice that every metric space is Hausdorff, so analogously, we can construct a complete metric space using an isometric embedding.
...
\end{dfnbox}
To obtain a compactification, we require quite a number of conditions. However, we will show that a metric completion always exists, and it is in fact unique up to isometry.
