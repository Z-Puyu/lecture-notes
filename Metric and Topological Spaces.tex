...
\begin{probox}{Normalisation Preserves Totally Bounded Spaces}{normaliseTotalBound}
...
\end{probox}
Similarly, bi-Lipschitz spaces should also preserve totally bounded properties.
...
In elementary real analysis, we examine the notion of convergence with Cauchy sequences. Recall that in $\R^n$, every Cauchy sequence is convergent. However, in general metric spaces, this might not be the case. We now generalise this idea to all metric spaces.
...
\end{probox}
Using these concepts, we shall re-visit the Euclidean space $\R^n$. First, let us argue that $\R^n$ is complete with respect to the $\ell_p$-metric for $p \geq 1$.
...
\end{probox}
With this result, it is easy to see that for all $n \in \N^+$, we have $x_n \in \widebar{B_M\left(0\right)}$ for some $M > 0$. Note that $\widebar{B_M\left(0\right)}$ is a compact ball in $\R^n$ and so it must be sequentially compact by Proposition \ref{pro:metricCompact}. Therefore, $\left\{x_n\right\}_{n \in \N^+}$ has a convergent subsequence and so it is convergent by Proposition \ref{pro:cauchyConverge}. Therefore, $\R^n$ is indeed complete with respect to the $\ell_p$-metric for $p \geq 1$.

However, if we restrict the standard metric to $\Q$, then $\Q$ is not complete because we can find a Cauchy sequence in $\Q$ which converges to some $x \in \Q^c$.

Lastly, we propose a more complex example:
\begin{probox}{Completeness of Infinite Product Spaces}{infProductComp}
...
\begin{corbox}{Heine-Borel Theorem}{HeineBorel}
...
It is easy to see that $\R^n$ is locally compact but $\Q$ is not locally compact because $\Q^c$ is dense.

We can also show that $\R^{\omega}$ with the product topology is not locally compact.
...
\begin{probox}{Compactification of Hausdorff Spaces}{compactificationHausdorff}
    Let $X$ be a Hausdorff space and $Y$ be a compact Hausdorff space. If a map $h_Y \colon X \to Y$ is homeomorphic onto $h_Y\left(X\right)$ and $Y \setminus h_Y\left(X\right)$ is a singleton set, then $Y$ is a compactification of $X$.
\end{probox}
...
\begin{thmbox}{Existence and Uniqueness of One-Point Compactification}{existenceUniquenessOnePt}
   Every locally compact and Hausdorff space $X$ has a one-point compactification $Y$ with respect to a map $h_Y$. Furthermore, if $Y'$ is another one-point compactification of $X$ with respect to the map $h_{Y'}$, then there exists a homeomorphism $f \colon Y \to Y'$ such that 
    \begin{equation*}
...
\begin{probox}{Locally Compact Hausdorff Spaces and Homeomorphisms}{characteriseLocallyCompactHausdorffHomeomorphism}
...
\begin{notebox}
    \begin{remark}
        The uniform topology on $\R^{\Lambda}$ is finer than the product topology and coarser than the box topology. If $\Lambda$ is infinite, then the three topologies are pair-wise unequal.
    \end{remark}
\end{notebox}
...
\end{dfnbox}
This topology is so named due to the fact that $\left\{f_n\right\}_{n \in \N}$ in $Y^X$ converges to $f$ with respect to the topology if and only if $\left\{\left. f_n \right\rvert_C\right\}_{n \in \N}$ converges to $\left. f\right\rvert_C$ uniformly for any compact set $C \subseteq X$.
