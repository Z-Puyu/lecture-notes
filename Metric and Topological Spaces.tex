...
In elementary real analysis, we examine the notion of convergence with Cauchy sequences. We now generalise this idea to all metric spaces.
...
\end{dfnbox}
We can first prove the following properties of Cauchy sequences:
\begin{probox}{Properties of Cauchy Sequences}{propsCauchy}
    \begin{enumerate}
        \item A Cauchy sequence converges if and only if it has a convergent subsequence.
        \item If $\left(X, d\right)$ and $\left(X', d'\right)$ are bi-Lipschitz, then a sequence is Cauchy in $\left(X, d\right)$ if and only if it is Cauchy in $\left(X', d'\right)$.
    \end{enumerate}
\end{probox}
