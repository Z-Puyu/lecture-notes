...
\end{dfnbox}
Let $\lambda$ be an eigenvalue of $\mathbfit{A}$, then there is some vector $\mathbfit{x} \neq \zero$ such that $\mathbfit{Ax} = \lambda\mathbfit{x}$. If $\mathbfit{A} = \mathbfit{P}^{-1}\mathbfit{B}\mathbfit{P}$, this means that 
\begin{equation*}
...
Now, let $T$ be any operator on $V$ and $z \colon \mathcal{F}^n \to V$ be a basis, then we know that the operator $T^*_z \coloneqq z^{-1} \circ T \circ z$ on $\mathcal{F}^n$ has the same eigenvalues as $T$.
