...
\begin{thmbox}{Composition Preserves Isomorphism}{compoPreserveIsomorphic}
    ...
    \end{proof}
\end{thmbox}
Using Theorem \ref{thm:compoPreserveIsomorphic}, we can immediately see that if $\phi \colon V \to V$ is any isomorphism and~$\psi \colon V \to U$ is another isomorphism, then $\psi \circ \phi$ is an isomorphism between $V$ and $U$. Therefore, there are infinitely many isomorphisms between $V$ and $U$.
\begin{notebox}
    \begin{remark}
...
\begin{dfnbox}{Dual Space}{dualSpace}
    Let $V$ be a vector space over $\F$. The {\color{red} \textbf{dual space}} of $V$ is defined as $V^* \coloneqq \mathcal{L}(V, \F)$. The elements of $V^*$ are called {\color{red} \textbf{dual vectors}}.
\end{dfnbox}
An intuitive example for an element of $V^*$ is as follows: let $\left\{\mathbfit{z}_1, \mathbfit{z}_2, \cdots, \mathbfit{z}_n\right\}$ be a basis for $V$. For any $\mathbfit{v} \in V$, we can write
\begin{equation*}
    \mathbfit{v} = \sum_{i = 1}^{n}a_i\mathbfit{z}_i
\end{equation*}
for $a_1, a_2, \cdots, a_n \in \F$. Now, define a mapping $\zeta^i \colon V \to \F$ as $\zeta^i(\mathbfit{v}) = a_i$, then clearly we have $\zeta^i \in V^*$ for $i = 1, 2, \cdots, n$. In particular, we see that
\begin{equation*}
...
\begin{dfnbox}{Dual Basis}{dualBasis}
    Let $V$ be a vector space with a basis $\left\{\mathbfit{z}_1, \mathbfit{z}_2, \cdots, \mathbfit{z}_n\right\}$. The {\color{red} \textbf{dual basis}} of $\left\{\mathbfit{z}_1, \mathbfit{z}_2, \cdots, \mathbfit{z}_n\right\}$ is defined as the set of all $\zeta^i \in V^*$ such that 
    \begin{equation*}
        \zeta^i(\mathbfit{z}_j) = \begin{cases}
            1 & \textrm{if } i = j \\
            0 & \textrm{otherwise}
    ...
    \end{equation*}
\end{dfnbox}
Let $\zeta$ be a dual basis and $\alpha \in V^*$ be a linear mapping over a finite-dimensional vector space~$V$ with $\dim(V) = n$. For any $\mathbfit{v} \in V$ with respect to basis $z$, let us define a mapping~$\beta \colon V \to \F$ by 
\begin{equation*}
...
\begin{align*}
    ...
    & = \alpha(\mathbfit{z}_j).
\end{align*}
Since $z$ is a basis for $V$, this implies that for any $\mathbfit{v} \in V$, we have $\beta(\mathbfit{v}) = \alpha(\mathbfit{v})$. Therefore, it should be clear that $\alpha \in \mathrm{span}(\zeta)$. 

Consider
\begin{equation*}
    \sum_{i = 1}^{n}p_i\zeta^i = 0_{V^*},
\end{equation*}
...
\begin{probox}{Dimension of Dual Space}{dualHasEqualDim}
    Let $V$ be any finite-dimensional vector space, then $\dim(V) = \dim\left(V^*\right)$.
\end{probox}
...
\end{equation*}
Recall that in Definition \ref{dfn:canonicalBasis}, we defined the canonical basis of $\F^n$ to be the set of unit vectors with a single non-zero entry. Similarly, we can define the canonical basis for the dual space $\left(\F^n\right)^* \coloneqq \mathcal{L}(\F^n, \F)$.
...
\end{dfnbox}
Consider any $p \in \left(\F^n\right)^*$, then we can write
\begin{equation*}
    p = \sum_{i = 1}^{n}q_i\epsilon^i
\end{equation*} 
where $q_i \in \F$ for $i = 1, 2, \cdots, n$ and $\epsilon^i$'s are the dual canonical basis. Recall that a basis of an $n$-dimensional vector space $V$ is actually a mapping from $\F^n$ to $V$, so it follows that the dual basis is in fact a mapping $\zeta \colon \left(\F^n\right)^* \to V^*$. We claim that this $\zeta$ is in fact just a mapping that satisfies 
\begin{equation*}
    \zeta(p)\bigl(z(\mathbfit{a})\bigr) = p(\mathbfit{a})
\end{equation*}
for any $p \in \left(\F^n\right)^*$ and $\mathbfit{a} \in \F^n$. To prove that our new definition is consistent with Definition \ref{dfn:dualBasis}, it suffices to show that $\zeta(\epsilon^i) = \zeta^i$. Notice that
\begin{align*}
...
\begin{align*}
    ...
    & = \zeta^i(\mathbfit{v}).
\end{align*}
Therefore, $\zeta(\epsilon^i) = \zeta^i$. Let $\phi \in V^*$ be a mapping, then
\begin{align*}
    ...
    & = \zeta(p)
\end{align*}
for some $p \in \left(\F^n\right)^*$. Therefore, $\zeta$ is surjective. One may check that $\zeta$ is injective. Moreover, note that for $p, q \in \left(\F^n\right)^*$,
\begin{align*}
...
\end{align*}
Therefore, $\zeta$ is a linear mapping and so it is an isomorphism. Indeed, this implies that $\zeta$ is a basis for $V^*$.

Naturally, it feels justified to define a ``dual space of the dual space'' of $V$, namely 
\begin{equation*}
    V^{**} \coloneqq \mathcal{L}\left(V^*, \F\right) = \mathcal{L}\bigl(\mathcal{L}(V, \F), \F\bigr).
\end{equation*}
...
\begin{probox}{Double Dual Space}{doubleDual}
    ...
    \tcblower
    \begin{proof}
        Define a mapping $K \colon V \to V^{**}$ by $K(\mathbfit{v}) = f$ where $f \colon V^* \to V$ is such that~$f(\alpha) = \alpha(\mathbfit{v})$. It suffices to prove that $K$ is an isomorphism. First, notice that for any~$\mathbfit{u}, \mathbfit{v} \in V$ and any $c \in \F$,
        \begin{align*}
        ...
        \end{align*}
        for all $\alpha \in V^*$. Therefore, $K$ is a homomorphism. Let $K(\mathbfit{v}) = 0_{V^* \to V}$ be the zero mapping, then 
        \begin{equation*}
            K(\mathbfit{v})(\alpha) = \alpha(\mathbfit{v}) = 0
        \end{equation*}
        for all $\alpha \in V^*$. This implies that $\mathbfit{v} = \zero$, and so $\mathrm{null}(K) = 0$ since $V$ is finite-dimensional. By Proposition \ref{pro:injectTest}, $K$ is injective, and so by Corollary \ref{cor:bijectiveReflexiveMap}, $K$ is bijective. Therefore, $K$ is an isomorphism and so $V^{**} \cong V$.
    \end{proof}
\end{probox}
Therefore, this means that it is perfectly legal to view each $\mathbfit{v} \in V$ as a mapping from $V^*$ to~$\F$. We can verify that $\mathbfit{v}$ is linear, because
\begin{align*}
    \mathbfit{v}(m\alpha + n\beta) & = m\alpha(\mathbfit{v}) + n\beta(\mathbfit{v}) \\
    & = m\mathbfit{v}(\alpha) + n\mathbfit{v}(\beta).
\end{align*}
Therefore, we can in some sense view a vector space and its dual space the \textit{dual} of each other. Consider a mapping $T \colon V \to V$ for some vector space $V$, the duality between $V$ and~$V^*$ tempts us to think that there exists some mapping $S \colon V^* \colon V^*$ with a certain correspondence to $T$.
\begin{dfnbox}{Transpose}{tranpose}
    Let $T \colon V \to V$ be a linear transformation. The {\color{red} \textbf{transpose}} of $T$ is defined as the mapping~$T^* \colon V^* \to V^*$ such that 
    \begin{equation*}
        T^*(\alpha)(\mathbfit{v}) = \alpha\bigl(T(\mathbfit{v})\bigr)
    \end{equation*}
    for any $\alpha \in V^*$. 
\end{dfnbox}
We can verify that the transpose is still linear, because for any $\alpha, \beta \in V^*$ and any $c \in \F$, we have 
\begin{align*}
    T^*(c\alpha + \beta)(\mathbfit{v}) & = (c\alpha + \beta)\bigl(T(\mathbfit{v})\bigr) \\
    & = c\alpha\bigl(T(\mathbfit{v})\bigr) + \beta\bigl(T(\mathbfit{v})\bigr) \\
    & = cT^*(\alpha)(\mathbfit{v}) + T^*(\beta)(\mathbfit{v}) \\
    & = \left(cT^*(\alpha) + T^*(\beta)\right)(\mathbfit{v})
\end{align*}
for all $\mathbfit{v} \in V$, which implies that $T^*(c\alpha + \beta) = cT^*(\alpha) + T^*(\beta)$.
...
\begin{probox}{Linearity of Transposition}{linearTranspose}
    Let $\tau$ be a mapping over $\mathcal{L}(V, V)$ for some finite-dimensional vector space $V$ such that~$\tau(T) = T^*$, then $\tau$ is a linear transformation.
    \tcblower
    \begin{proof}
        Take any $\alpha, \beta \in \mathcal{V, V}$ and any $c \in \F$. For any $\gamma \in \mathcal{L}(V, V)$ and any $\mathbfit{v} \in V$, consider 
        \begin{align*}
            \tau(c\alpha + \beta)(\gamma)(\mathbfit{v}) & = \left(c\alpha + \beta\right)^*(\gamma)(\mathbfit{v}) \\
            & = \gamma\bigl((c\alpha + \beta)(\mathbfit{v})\bigr) \\
            & = c\gamma\bigl(\alpha(\mathbfit{v})\bigr) + \gamma\bigl(\beta(\mathbfit{v})\bigr) \\
            & = c\alpha^*(\gamma)(\mathbfit{v}) + \beta^*(\gamma)(\mathbfit{v}) \\
            & = c\tau(\alpha)(\gamma)(\mathbfit{v}) + \tau(\beta)(\gamma)(\mathbfit{v}).
        \end{align*}
        Since $\mathbfit{v}$ is arbitrary, $\tau(c\alpha + \beta)(\gamma) = c\tau(\alpha)(\gamma) + \tau(\beta)(\gamma)$ for all $\gamma \in \mathcal{L}(V, V)$, and so this implies that $\tau(c\alpha + \beta) = c\tau(\alpha) + \tau(\beta)$. Therefore, $\tau$ is a linear transformation.
    \end{proof}
\end{probox}
...
A great usage of vector spaces is to help us generalise geometry from Euclidean spaces to abstract structures. In Euclidean spaces, we have many well-defined geometric notions such as length, area, angle and volume. However, our ultimate aim is to generalise such geometry to any kind of vector spaces. We will first take the \textit{dot product} as an example to see how we can apply an axiomatic approach to abstract Euclidean geometry for general vector spaces.
...
Bilinear maps motivates the notion of \textit{tensor products}.
\begin{dfnbox}{Tensor Product}{tensor}
    Let $V$ and $W$ be vector spaces. The {\color{red} \textbf{tensor product}} of $V$ and $W$, denoted by $V \otimes W$, is a vector space with a bilinear map $\otimes \colon V \times W \to V \otimes W$ such that for every bilinear map $h \colon V \times W \to Z$, there is a unique map $\widetilde{h} \colon V \otimes W \to Z$ such that $h = \widetilde{h} \circ \otimes$.
\end{dfnbox}
While tensor products are useful constructions, the form in which they are represented can be overly complicated and obscure. Therefore, it would be useful to consider some concrete examples of tensor products.
\begin{dfnbox}{Tensor Product between Vector Spaces and Their Dual}{tensorProd}
    Let $V$ be a vector space with the dual space $V^*$. For any $\mathbfit{v} \in V$ and $\alpha \in V^*$, their {\color{red} \textbf{tensor product}} is defined as a mapping $\mathbfit{v} \otimes \alpha \in \mathcal{L}(V, V)$ such that
    \begin{equation*}
        (\mathbfit{v} \otimes \alpha)(\mathbfit{u}) = \alpha(\mathbfit{u})\mathbfit{v}.
    \end{equation*}
\end{dfnbox}
It might be useful to check that this definition is consistent. Let us consider the vector space~$V \otimes V^*$. For every $\mathbfit{u} \in V$, we define $h_{\mathbfit{u}} \colon V \times V^* \to V$ to be a bilinear map such that~$h_{\mathbfit{u}}(\mathbfit{v}, \alpha) = \alpha(\mathbfit{v})\mathbfit{u}$. By Definition \ref{dfn:tensor}, there is a unique map $\widetilde{h}_{\mathbfit{u}} \colon V \otimes V^* \to V$ such that 
\begin{equation*}
...
\end{align*}
In particular, let $\beta \in V^*$, then clearly $\beta \circ (\mathbfit{v} \otimes \alpha) \in V^*$. Define a mapping $T$ over $V^*$ by
\begin{equation*}
    T(\beta) = \beta \circ (\mathbfit{v} \otimes \alpha),
\end{equation*}
then we can see that $T \in \mathcal{L}\left(V^*, V^*\right)$. In fact, for any $\mathbfit{u} \in V$, consider
\begin{align*}
...
\end{align*}
By Definition \ref{dfn:tranpose}, $T$ is the transpose of $\mathbfit{v} \otimes \alpha$ for any $\mathbfit{v} \in V$ and $\alpha \in V^*$. Furthermore, note that $\alpha(\mathbfit{u}) \in \F$, we see that
\begin{align*}
...
Amazingly, such tensor products turn out to be closely related to the basis of $\mathcal{L}(V, V)$.
...
\end{equation*}
In the following part, we will demonstrate how tensor products are actually tied closely to an old classic concept --- matrices!
...
\end{dfnbox}
From Proposition \ref{pro:tenserProdBasis}, the components of a mapping $T \in \mathcal{L}(V, V)$ can be essentially seen as the entries of a matrix.
...
\begin{probox}{Matrix Representation of Transpose Mappings}{matrixTranspose}
    Let $T \colon V \to V$ be a linear mapping with matrix representation $\mathbfit{T}$. Let $T^*$ be the transpose of $T$, then the matrix representation of $T^*$ is such that $\left(T^*\right)^{i}_j = T^j_i$.
    \tcblower
    \begin{proof}
        Define $K \colon V \to V^{**}$ by $K(\mathbfit{v})(\alpha) = \alpha(\mathbfit{v})$, then $K(\mathbfit{v}) = \alpha \mapsto \alpha(\mathbfit{v})$. Consider the following lemma:
        \begin{lembox}{Transpose of Tensor Products}{tensorTrans}
            Let $V$ be a finite-dimensional space. For any $\mathbfit{v} \in V$ and $\alpha \in V^*$, we have 
            \begin{equation*}
                \left(\mathbfit{v} \otimes \alpha\right)^* = \alpha \otimes K(\mathbfit{v}).
            \end{equation*}
            \tcblower
            \begin{proof}
                Let $f \colon \mathbfit{v} \otimes \alpha$, for any $\beta \in V^*$ and any $\mathbfit{w} \in V$, consider 
                \begin{align*}
                    f^*(\beta)(\mathbfit{w}) & = \beta\bigl(f(\mathbfit{w})\bigr) \\
                    ...
                    & = \alpha(\mathbfit{w})\beta(\mathbfit{v}).
                \end{align*}     
                Therefore, $f^*(\beta) =  \bigl(\alpha \otimes K(\mathbfit{v})\bigr)(\beta)$ for all $\beta \in V^*$, which implies that 
                \begin{equation*}
                    \mathbfit{v} \otimes \alpha = f^* = \alpha \otimes K(\mathbfit{v}).
                \end{equation*}
            \end{proof}
        \end{lembox}
        ...
        \begin{equation*}
            ...
            \end{cases}.
        \end{equation*}
        Therefore, all of the $K(\mathbfit{z}_i)$'s forms a dual basis for $V^*$. By Proposition \ref{pro:tenserProdBasis} and Definition \ref{dfn:linTransMat}, we know that 
        \begin{equation*}
            T^* = \sum_{i = 1}^{n}\sum_{j = 1}^{n}\left(T^*\right)^i_j\left(\zeta^i \otimes K(\mathbfit{z}_j)\right), \qquad T = \sum_{j = 1}^{n}\sum_{i = 1}^{n}T^j_i\left(\mathbfit{z}_j \otimes \zeta^i\right).
        \end{equation*}
        By Proposition \ref{pro:linearTranspose}, we know that 
        \begin{equation*}
            T^* = \sum_{j = 1}^{n}\sum_{i = 1}^{n}T^j_i\left(\left(\mathbfit{z}_j \otimes \zeta^i\right)^*\right).
        \end{equation*}
        By Lemma \ref{lem:tensorTrans}, $\left(\mathbfit{z}_j \otimes \zeta^i\right)^* = \zeta^i \otimes K(\mathbfit{z}_j)$, so $\left(T^*\right)^i_j = T^j_i$. 
    \end{proof}
\end{probox}
...
Note that we can augment $\mathcal{L}(V, V)$ with the $\circ$ operation. For any $S, T \in \mathcal{L}(V, V)$, it is clear that~$S \circ T \in \mathcal{L}(V, V)$, so $\mathcal{L}(V, V)$ is an algebra. This means that it is justified to write $S \circ T$ as a product $ST$.

Let $\mathbfit{u}, \mathbfit{v} \in V$ and $\alpha, \beta \in V^*$, for any $\mathbfit{w} \in V$, consider
\begin{align*}
...
\begin{probox}{Matrices of Composite Mappings}{matComp}
    ...
    \end{proof}
\end{probox}
Let $z$ be a basis for a vector space $V$. For any $\mathbfit{v} \in V$, define $\mathbfit{v}^*_z \coloneqq z^{-1}(\mathbfit{v})$. Clearly $\mathbfit{v}^*_z \in \F^n$. We can view $\mathbfit{v}^*_z$ as the ``coordinates'' of $\mathbfit{v}$ with respect to $z$. Suppose $\alpha \in V^*$, we define some $\alpha^*_z \in \left(\F^n\right)^*$ by $\alpha^*_z \coloneqq \alpha \circ z$. In particular, suppose the $i$-th component of $\alpha$ is $\alpha_i$, we notice that
\begin{equation*}
...
\end{equation*}
Combining everything, we have the following definition:
...
\end{dfnbox}
So far, by simply considering tensor products between a vector space and its dual, we have already obtained so many amazing results. Now let us delve deeper and apply tensor product to two dual spaces.
\begin{dfnbox}{Tensor Product between Dual Vectors}{dualVecTensor}
    Let $V$ be a finite-dimensional vector space. For any $\alpha, \beta \in V^*$, their {\color{red} \textbf{tensor product}} is defined as the mapping 
    \begin{equation*}
        \alpha \otimes \beta \colon V \times V \to \F
    \end{equation*}
    such that $(\alpha \otimes \beta)(\mathbfit{u}, \mathbfit{v}) = \alpha(\mathbfit{u})\beta(\mathbfit{v})$.
\end{dfnbox}
We shall also check that this definition stays consistent with the universal property in Definition \ref{dfn:tensor}. For each $\mathbfit{u}, \mathbfit{v} \in V$, define $h_{\mathbfit{u}, \mathbfit{v}} \colon V^* \times V^* \to \F$ by $h_{\mathbfit{u}, \mathbfit{v}}(\alpha, \beta) = \alpha(\mathbfit{u})\beta(\mathbfit{v})$. Now, there is a unique map $\widetilde{h}_{\alpha, \beta} \colon V^* \otimes V^* \to \F$ such that 
\begin{equation*}
...
\end{equation*}
If we take $\otimes \colon V^* \times V^* \to \mathcal{L}\left(V^2, \F\right)$, clearly $\widetilde{h}_{\mathbfit{u}, \mathbfit{v}}(\alpha \otimes \beta) = (\alpha \otimes \beta)(\mathbfit{u}, \mathbfit{v}) = \alpha(\mathbfit{u})\beta(\mathbfit{v})$.

It is easy to see that for any basic vectors $\zeta^i, \zeta^j$ for $V^*$, the mapping $\zeta^i \otimes \zeta^j$ is a bilinear form over $V$.
\begin{probox}{Basis of the Bilinear Form Space}{bilBasis}
    ...
    \end{proof}
\end{probox}
In this sense, $\mathbb{B}(V) = \mathcal{L}(V^2, \F)$. However, intuitively for each mapping $(\mathbfit{u}, \mathbfit{v}) \mapsto c$, this is equivalent to finding some $T_{\mathbfit{u}} \in V^*$ for each $\mathbfit{u} \in V$ and apply the mapping to $\mathbfit{v}$.
\begin{probox}{Every Bilinear Maps to a Unique Dual Vector}{BisLVV*}
    For any finite-dimensional vector space $V$, we have $\mathbb{B}(V) \cong \mathcal{L}\left(V, V^*\right)$.
    \tcblower
    \begin{proof}
        Take any $b \in \mathbb{B}(V)$. Define a mapping $f \colon \mathbb{B}(V) \to  \mathcal{L}\left(V, V^*\right)$ by $f(b) = b^{\#}$, where $b^{\#}(\mathbfit{v})(\mathbfit{u}) = b(\mathbfit{u}, \mathbfit{v})$. Suppose there are $b_1, b_2 \in \mathbb{B}(V)$ with $b_1 \neq b_2$, then there exists $\mathbfit{u}, \mathbfit{v} \in V$ such that 
        \begin{equation*}
            b_1^{\#}(\mathbfit{v})(\mathbfit{u}) = b_1(\mathbfit{u}, \mathbfit{v}) \neq b_2(\mathbfit{u}, \mathbfit{v}) = b_2^{\#}(\mathbfit{v})(\mathbfit{u}),
        \end{equation*}
        which implies that $b_1^{\#}(\mathbfit{v}) \neq b_2^{\#}(\mathbfit{v})$, and so $b_1^{\#} \neq b_2^{\#}$. Therefore, the $f$ is injective. Now, take any $b^{\#} \in \mathcal{L}\left(V, V^*\right)$, let $b$ be a bilinear form with $b(\mathbfit{u}, \mathbfit{v}) = b^{\#}(\mathbfit{v})(\mathbfit{u})$, then by definition clearly $f(b) = b^{\#}$. Therefore, $f$ is surjective, and so $\mathbb{B}(V) \cong \mathcal{L}\left(V, V^*\right)$.
    \end{proof}
\end{probox}
We can write 
\begin{equation*}
...
\begin{notebox}
    \begin{remark}
        Clearly, equality is attained in the triangle inequality if and only if $\mathbfit{u} \parallel \mathbfit{v}$ as well due to the use of Cauchy-Schwartz inequality.
    \end{remark}
\end{notebox}
...
\begin{dfnbox}{Angle}{angle}
    ...
    \end{equation*}
\end{dfnbox}
Note that this definition is only solid with Theorem \ref{thm:CauchySchwarz} established, which helps ensure that~$\cos\theta \in [-1, 1]$ here.

\chapter{Linear Transformations}
\section{Eigenspaces}
Since there are infinitely many linear transformations between two vectors spaces, it is not of much help to study each one of them individually. Instead, it is useful to categorise all linear transformations into different families according to some well-defined properties. First, we consider a simple form of linear transformation which maps a set back into itself.
...
\end{dfnbox}
Note that the operator $T$ is a well-defined function, so for every eigenvector, its associated eigenvalue is unique. Such uniqueness guarantees the following contrapositive:
...
\begin{probox}{Existence of Eigenvalue}{eigenvalExists}
    ...
    \begin{proof}
        ...
        \end{equation*}
        for some $a_0, a_1, \cdots, a_n \in \C$. Note that not all of the $(T - a_i\mathrm{id}_V)$'s are injective, because otherwise $\mathbfit{v} = \zero$ by Proposition \ref{pro:injectTest}. Therefore, there is some $T - a_j\mathrm{id}_V$ which is not injective. This implies that there must exist a non-zero vector $\mathbfit{w} \in V$ such that~$(T - a_j\mathrm{id}_V)(\mathbfit{w}) = \zero$, but this implies that $T(\mathbfit{w}) = a_j\mathbfit{w}$, so $a_j$ is an eigenvalue of~$T$ associated with $\mathbfit{w}$.
    \end{proof}
\end{probox}
...
\end{notebox}
    ...
    Recall that in Proposition \ref{pro:BisLVV*}, we have demonstrated that every bilinear form over a vector space $V$ defines a mapping from $V$ to $V^*$. This mapping becomes very special if we construct it with respect to an inner product.
    \begin{thmbox}{Riesz Representation Theorem}{Riesz}
        For any inner product space $(V, g)$, let the mapping $\Gamma \colon V \to V^*$ be such that
        \begin{equation*}
            \Gamma(\mathbfit{u})(\mathbfit{v}) = g(\mathbfit{v}, \mathbfit{u}),
        \end{equation*}
        then for every $\alpha \in V^*$, there is a unique $\mathbfit{u}_{\alpha} \in V$ such that $\alpha = \Gamma\left(\mathbfit{u}_{\alpha}\right)$.
        \tcblower
        \begin{proof}
            ...
            \begin{equation*}
                \mathrm{rank}(\Gamma) = \dim(V) = \dim\left(V^*\right),
            \end{equation*}
            and so $\mathrm{range}(\Gamma) = V^*$, which means that $\Gamma$ is surjective. Therefore, $\Gamma$ is an isomorphism.
        \end{proof}
    \end{thmbox}
    ...
    \begin{probox}{Conversion from Operators to Bilinear Forms}{convertOpToBil}
        ...
        \tcblower
        \begin{proof}
            By Proposition \ref{pro:BisLVV*}, since $\Gamma \circ T \in \mathcal{L}\left(V, V^*\right)$, there exists a unique bilinear form~$b_T \colon V \times V \to V$ defined by 
            \begin{equation*}
            ...
            \end{equation*}
            Conversely, for any bilinear form $b \in \mathbb{B}(V)$, we can similarly construct $b^{\#}_T \colon V \to V^*$ by $b^{\#}_T(\mathbfit{v})(\mathbfit{u}) = b(\mathbfit{u}, \mathbfit{v})$. Therefore, $\Gamma^{-1} \circ b^{\#}_T \in \mathcal{L}(V, V)$ is an operator over $V$. 
        \end{proof}
    \end{probox}
    ...
    \end{dfnbox}
    If we denote the set of all sesquilinear forms over a vector space $V$ as $\mathbb{S}(V)$, then just like Proposition \ref{pro:BisLVV*}, $\mathbb{S}(V) \cong \mathrm{Maps}\left(V, V^*\right)$. This is because for every $s \in \mathbb{S}(V)$, we can also define some $s^{\#} \in \mathrm{Maps}\left(V, V^*\right)$ such that $s^{\#}(\mathbfit{v})(\mathbfit{u}) = s(\mathbfit{u}, \mathbfit{v})$. However, notice that now, $s^{\#}$ is no longer linear. Instead, it is said to be \textit{conjugate-linear}.
    ...
    \end{dfnbox}
    Therefore, every sesquilinear map over a complex vector space $V$ defines a unique conjugate-linear map from $V$ to $V^*$. Conversely, for any conjugate-linear map $s^{\#} \colon V \to V^*$, we can define some sesquilinear form $s \colon V \times V \to \C$ by $s(\mathbfit{u}, \mathbfit{v}) = s^{\#}(\mathbfit{v})(\mathbfit{u})$.
    ...
    \begin{notebox}
        ...
        \end{remark}
    \end{notebox}
    In Theorem \ref{thm:Schur}, if our initial basis $y$ is already orthonormal, then it is clear that 
    \begin{equation*}
        \mathbfit{Q} = \mathbfit{G}^{-1}\mathbfit{M}^{-1}M_y(T)\mathbfit{M}\mathbfit{G},
    \end{equation*}
    and so $\mathbfit{M}\mathbfit{G}$ is unitary. This theorem also has a version specialised for sesquilinear forms.
    ...
    \begin{thmbox}{Spectral Theorem}{spectral}
        ...
        \begin{proof}
            ...
            \end{equation*}
            Hence, $\mathbfit{U}$ is Hermitian. However, this means that $\mathbfit{U}$ is both upper- and lower-triangular and so $\mathbfit{U}$ is diagonal. Notice that $U^i_i = \widebar{U^i_i}$, so $\mathbfit{U}$ must be a real matrix.
        \end{proof}
    \end{thmbox}
...
\begin{dfnbox}{Wedge Product}{wedge}
    Let $V$ be a finite-dimensional vector space. For every $\alpha, \beta \in V^*$, their {\color{red} \textbf{wedge product}} or {\color{red} \textbf{exterior product}} is defined as 
    \begin{equation*}
...
\begin{enumerate}
    \item For an $m$-form to be well-defined over an $n$-dimensional vector space, we require that~$0 < m \leq n$.
...
\end{equation*}
for some dual basis $\left\{\zeta^1, \zeta^2, \cdots, \zeta^n\right\}$. Recall that in Definition \ref{dfn:tranpose}, we defined the transposition of linear transformations. Applying this to $m$-forms, we can similarly define, for each linear operator $T$ on $V$, an operator $T^*$ on the space of $m$-forms over $V$ by
\begin{equation*}
    T^*(\Psi)\left(\mathbfit{v}_1, \mathbfit{v}_2, \cdots, \mathbfit{v}_m\right) = \Psi\bigl(T(\mathbfit{v}_1), T(\mathbfit{v}_2), \cdots, T(\mathbfit{v}_m)\bigr).
\end{equation*}
Now, if $V$ happens to be $m$-dimensional, then $T^*(\Psi)$ has to be some scalar multiple of $\Psi$ because the space of $m$-forms now becomes one-dimensional.
\section{Determinant}
\begin{dfnbox}{Determinant}{determinant}
    ...
    \begin{equation*}
        T^*(\Omega) = \Delta(T)\Omega
    \end{equation*}
    for every $n$-form $\Omega$ over $V$.
\end{dfnbox}
\begin{notebox}
    \begin{remark}
        Clearly, $\Delta(T)$ is the unique eigenvalue of $T^*$.
    \end{remark}
\end{notebox}
...
\begin{probox}{Computing Determinant of Operators}{computeDet}
    ...
    \begin{proof}
        ...
        \begin{align*}
            \Delta(T)\Omega & = T^*(\Omega)\left(\mathbfit{t}_1, \mathbfit{t}_2, \cdots, \mathbfit{t}_n\right) \\
...
\begin{probox}{Operator Composition Preserves Determinant}{detComp}
    ...
    \begin{proof}
        ...
        \begin{align*}
            \Delta(TS)\Omega\left(\mathbfit{v}_1, \mathbfit{v}_2, \cdots, \mathbfit{v}_{\dim(V)}\right) & = \yhwidehat{TS}(\Omega)\left(\mathbfit{v}_1, \mathbfit{v}_2, \cdots, \mathbfit{v}_{\dim(V)}\right) \\
            & = \Omega\Bigl(TS(\mathbfit{v}_1), TS(\mathbfit{v}_2), \cdots, TS\bigl(\mathbfit{v}_{\dim(V)}\bigr)\Bigr) \\
            & = T^*(\Omega)\Bigl(S(\mathbfit{v}_1), S(\mathbfit{v}_2), \cdots, S\bigl(\mathbfit{v}_{\dim(V)}\bigr)\Bigr) \\
            & = \Delta(T)\yhwidehat{S}(\Omega)\bigl(\mathbfit{v}_1, \mathbfit{v}_2, \cdots, \mathbfit{v}_{\dim(V)}\bigr) \\
