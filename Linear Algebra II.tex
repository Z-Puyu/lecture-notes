...
\begin{dfnbox}{Dual Space}{dualSpace}
    Let $V$ be a vector space over $\F$. The {\color{red} \textbf{dual space}} of $V$ is defined as $\yhwidehat{V} \coloneqq \mathcal{L}(V, \F)$. The elements of $\yhwidehat{V}$ are called {\color{red} \textbf{dual vectors}}.
\end{dfnbox}
\begin{notebox}
    \textbf{Author's Note}: in almost all fields of mathematics, the conventional notation for the dual space of $V$ is $V^*$, but somehow the lecturer of this course decides (yet again) to use a confusing syntax by writing it as $\yhwidehat{V}$.
\end{notebox}
An intuitive example for an element of $\yhwidehat{V}$ is as follows: let $\left\{\mathbfit{z}_1, \mathbfit{z}_2, \cdots, \mathbfit{z}_n\right\}$ be a basis for $V$. For any $\mathbfit{v} \in V$, we can write
\begin{equation*}
    \mathbfit{v} = \sum_{i = 1}^{n}a_i\mathbfit{z}_i
\end{equation*}
for $a_1, a_2, \cdots, a_n \in \F$. Now, define a mapping $\zeta^i \colon V \to \F$ as $\zeta^i(\mathbfit{v}) = a_i$, then clearly $\zeta^i \in \yhwidehat{V}$ for $i = 1, 2, \cdots, n$.
...
\begin{dfnbox}{Dual Basis}{dualBasis}
    Let $V$ be a vector space with a basis $\left\{\mathbfit{z}_1, \mathbfit{z}_2, \cdots, \mathbfit{z}_n\right\}$. The {\color{red} \textbf{dual basis}} of $\left\{\mathbfit{z}_1, \mathbfit{z}_2, \cdots, \mathbfit{z}_n\right\}$ is defined as the set of all $\zeta^i \in \yhwidehat{V}$ such that 
    \begin{equation*}
        \zeta^i(\mathbfit{z}_j) = \begin{cases}
            1 & \textrm{if } i = j \\
            0 & \textrm{otherwise}
    ...
    \end{equation*}
\end{dfnbox}
Let $\zeta$ be a dual basis and $\alpha \in \yhwidehat{V}$ be a linear mapping over a finite-dimensional vector space $V$ with $\dim(V) = n$. For any $\mathbfit{v} \in V$ with respect to basis $z$, define a mapping $\beta \colon V \to \F$ by 
\begin{equation*}
...
Consider
\begin{equation*}
    \sum_{i = 1}^{n}p_i\zeta^i = 0_{\yhwidehat{V}},
\end{equation*}
...
\begin{probox}{Dimension of Dual Space}{dualHasEqualDim}
    Let $V$ be any finite-dimensional vector space, then $\dim(V) = \dim\left(\yhwidehat{V}\right)$.
\end{probox}
...
\end{equation*}
Recall that in Definition \ref{dfn:canonicalBasis}, we defined the canonical basis of $\F^n$ to be the set of unit vectors with a single non-zero entry. Similarly, we can define the canonical basis for the dual space $\yhwidehat{\F^n} \coloneqq \mathcal{L}(\F^n, \F)$.
...
\begin{align*}
    ...
    & = \zeta^i(\mathbfit{v}).
\end{align*}
Therefore, $\zeta(\epsilon^i) = \zeta^i$. Let $\phi \in \yhwidehat{V}$ be a mapping, then
\begin{align*}
    ...
    & = \zeta(p)
\end{align*}
for some $p \in \yhwidehat{\F^n}$. Therefore, $\zeta$ is surjective. One may check that $\zeta$ is injective. Moreover, note that for $p, q \in \yhwidehat{\F^n}$,
\begin{align*}
...
\end{align*}
Therefore, $\zeta$ is a linear mapping and so it is an isomorphism. Indeed, this implies that $\zeta$ is a basis for $\yhwidehat{V}$.

Naturally, it feels justified to define a ``dual space of the dual space'' of $V$, namely 
\begin{equation*}
    \yhwidehat{\yhwidehat{V}} \coloneqq \mathcal{L}\left(\yhwidehat{V}, \F\right) = \mathcal{L}\bigl(\mathcal{L}(V, \F), \F\bigr).
\end{equation*}
...
\begin{probox}{Double Dual Space}{doubleDual}
    ...
    \tcblower
    \begin{proof}
        Define a mapping $K \colon V \to \yhwidehat{\yhwidehat{V}}$ by $K(\mathbfit{v}) = f$ where $f \colon \yhwidehat{V} \to V$ is such that $f(\alpha) = \alpha(\mathbfit{v})$. It suffices to prove that $K$ is an isomorphism. First, note that for any $\mathbfit{u}, \mathbfit{v} \in V$ and any $c \in \F$,
        \begin{align*}
        ...
        \end{align*}
        for all $\alpha \in \yhwidehat{V}$. Therefore, $K$ is a homomorphism. Let $K(\mathbfit{v}) = 0_{\yhwidehat{V} \to V}$ be the zero mapping, then 
        \begin{equation*}
            K(\mathbfit{v})(\alpha) = \alpha(\mathbfit{v}) = 0
        \end{equation*}
        for all $\alpha \in \yhwidehat{V}$. This implies that $\mathbfit{v} = \zero$, and so $\mathrm{null}(K) = 0$ since $V$ is finite-dimensional. By Proposition \ref{pro:injectTest}, $K$ is injective, and so by Corollary \ref{cor:bijectiveReflexiveMap}, $K$ is bijective. Therefore, $K$ is an isomorphism and so $\yhwidehat{\yhwidehat{V}} \cong V$.
    \end{proof}
\end{probox}
Therefore, this means that it is perfectly legal to view each $\mathbfit{v} \in V$ as a mapping from $\yhwidehat{V}$ to $\F$. We can verify that $\mathbfit{v}$ is linear, because
\begin{align*}
    \mathbfit{v}(m\alpha + n\beta) & = m\alpha(\mathbfit{v}) + n\beta(\mathbfit{v}) \\
    & = m\mathbfit{v}(\alpha) + n\mathbfit{v}(\beta).
\end{align*}
Therefore, we can in some sense view a vector space and its dual space the \textit{dual} of each other. Consider a mapping $T \colon V \to V$ for some vector space $V$, the duality between $V$ and~$\yhwidehat{V}$ tempts us to think that there exists some mapping $S \colon \yhwidehat{V} \colon \yhwidehat{V}$ with a certain correspondence to $T$.
\begin{dfnbox}{Transpose}{tranpose}
    Let $T \colon V \to V$ be a linear transformation. The {\color{red} \textbf{transpose}} of $T$ is defined as the mapping~$\yhwidehat{T} \colon \yhwidehat{V} \to \yhwidehat{V}$ such that 
    \begin{equation*}
        \yhwidehat{T}(\alpha)(\mathbfit{v}) = \alpha\bigl(T(\mathbfit{v})\bigr)
    \end{equation*}
    for any $\alpha \in \yhwidehat{V}$. 
\end{dfnbox}
We can verify that the transpose is still linear, because for any $\alpha, \beta \in \yhwidehat{V}$ and any $c \in \F$, we have 
\begin{align*}
    \yhwidehat{T}(c\alpha + \beta)(\mathbfit{v}) & = (c\alpha + \beta)\bigl(T(\mathbfit{v})\bigr) \\
    & = c\alpha\bigl(T(\mathbfit{v})\bigr) + \beta\bigl(T(\mathbfit{v})\bigr) \\
    & = c\yhwidehat{T}(\alpha)(\mathbfit{v}) + \yhwidehat{T}(\beta)(\mathbfit{v}) \\
    & = \left(c\yhwidehat{T}(\alpha) + \yhwidehat{T}(\beta)\right)(\mathbfit{v})
\end{align*}
for all $\mathbfit{v} \in V$, which implies that $\yhwidehat{T}(c\alpha + \beta) = c\yhwidehat{T}(\alpha) + \yhwidehat{T}(\beta)$.
...
\begin{probox}{Linearity of Transposition}{linearTranspose}
    Let $\tau$ be a mapping over $\mathcal{L}(V, V)$ for some finite-dimensional vector space $V$ such that $\tau(T) = \yhwidehat{T}$, then $\tau$ is a linear transformation.
    \tcblower
    \begin{proof}
        Take any $\alpha, \beta \in \mathcal{V, V}$ and any $c \in \F$. For any $\gamma \in \mathcal{L}(V, V)$ and any $\mathbfit{v} \in V$, consider 
        \begin{align*}
            \tau(c\alpha + \beta)(\gamma)(\mathbfit{v}) & = \left(\yhwidehat{c\alpha + \beta}\right)(\gamma)(\mathbfit{v}) \\
            & = \gamma\bigl((c\alpha + \beta)(\mathbfit{v})\bigr) \\
            & = c\gamma\bigl(\alpha(\mathbfit{v})\bigr) + \gamma\bigl(\beta(\mathbfit{v})\bigr) \\
            & = c\yhwidehat{\alpha}(\gamma)(\mathbfit{v}) + \yhwidehat{\beta}(\gamma)(\mathbfit{v}) \\
...
\begin{dfnbox}{Tensor Product between Vector Spaces and Their Dual}{tensorProd}
    Let $V$ be a vector space with the dual space $\yhwidehat{V}$. For any $\mathbfit{v} \in V$ and $\alpha \in \yhwidehat{V}$, their {\color{red} \textbf{tensor product}} is defined as a mapping $\mathbfit{v} \otimes \alpha \in \mathcal{L}(V, V)$ such that
    \begin{equation*}
...
\end{align*}
In particular, let $\beta \in \yhwidehat{V}$, then clearly $\beta \circ (\mathbfit{v} \otimes \alpha) \in \yhwidehat{V}$. Define a mapping $T$ over $\yhwidehat{V}$ by
\begin{equation*}
    T(\beta) = \beta \circ (\mathbfit{v} \otimes \alpha),
\end{equation*}
then we can see that $T \in \mathcal{L}\left(\yhwidehat{V}, \yhwidehat{V}\right)$. In fact, for any $\mathbfit{u} \in V$, consider
\begin{align*}
...
\end{align*}
By Definition \ref{dfn:tranpose}, $T$ is the transpose of $\mathbfit{v} \otimes \alpha$ for any $\mathbfit{v} \in V$ and $\alpha \in \yhwidehat{V}$. Furthermore, note that $\alpha(\mathbfit{u}) \in \F$, we see that
\begin{align*}
...
\begin{probox}{Matrix Representation of Transpose Mappings}{matrixTranspose}
    Let $T \colon V \to V$ be a linear mapping with matrix representation $\mathbfit{T}$. Let $\yhwidehat{T}$ be the transpose of $T$, then the matrix representation of $\yhwidehat{T}$ is such that $\yhwidehat{T}^{i}_j = T^j_i$.
    \tcblower
    \begin{proof}
        Define $K \colon V \to \yhwidehat{\yhwidehat{V}}$ by $K(\mathbfit{v})(\alpha) = \alpha(\mathbfit{v})$, then $K(\mathbfit{v}) = \alpha \mapsto \alpha(\mathbfit{v})$. Consider the following lemma:
        \begin{lembox}{Transpose of Tensor Products}{tensorTrans}
            Let $V$ be a finite-dimensional space. For any $\mathbfit{v} \in V$ and $\alpha \in \yhwidehat{V}$, we have 
            \begin{equation*}
                \yhwidehat{\mathbfit{v} \otimes \alpha} = \alpha \otimes K(\mathbfit{v}).
            \end{equation*}
            \tcblower
            \begin{proof}
                Let $f \colon \mathbfit{v} \otimes \alpha$, for any $\beta \in \yhwidehat{V}$ and any $\mathbfit{w} \in V$, consider 
                \begin{align*}
                    \yhwidehat{f}(\beta)(\mathbfit{w}) & = \beta\bigl(f(\mathbfit{w})\bigr) \\
                    ...
                    & = \alpha(\mathbfit{w})\beta(\mathbfit{v}).
                \end{align*}     
                Therefore, $\yhwidehat{f}(\beta) =  \bigl(\alpha \otimes K(\mathbfit{v})\bigr)(\beta)$ for all $\beta \in \yhwidehat{V}$, which implies that 
                \begin{equation*}
                    \mathbfit{v} \otimes \alpha = \yhwidehat{f} = \alpha \otimes K(\mathbfit{v}).
                \end{equation*}
            \end{proof}
        \end{lembox}
        ...
        \begin{equation*}
            ...
            \end{cases}.
        \end{equation*}
        Therefore, all of the $K(\mathbfit{z}_i)$'s forms a dual basis for $\yhwidehat{V}$. By Proposition \ref{pro:tenserProdBasis} and Definition \ref{dfn:linTransMat}, we know that 
        \begin{equation*}
            \yhwidehat{T} = \sum_{i = 1}^{n}\sum_{j = 1}^{n}\yhwidehat{T}^i_j\left(\zeta^i \otimes K(\mathbfit{z}_j)\right), \qquad T = \sum_{j = 1}^{n}\sum_{i = 1}^{n}T^j_i\left(\mathbfit{z}_j \otimes \zeta^i\right).
        \end{equation*}
        By Proposition \ref{pro:linearTranspose}, we know that 
        \begin{equation*}
            \yhwidehat{T} = \sum_{j = 1}^{n}\sum_{i = 1}^{n}T^j_i\left(\yhwidehat{\mathbfit{z}_j \otimes \zeta^i}\right).
        \end{equation*}
        By Lemma \ref{lem:tensorTrans}, $\yhwidehat{\mathbfit{z}_j \otimes \zeta^i} = \zeta^i \otimes K(\mathbfit{z}_j)$, so $\yhwidehat{T}^i_j = T^j_i$. 
    \end{proof}
\end{probox}
...
Let $\mathbfit{u}, \mathbfit{v} \in V$ and $\alpha, \beta \in \yhwidehat{V}$, for any $\mathbfit{w} \in V$, consider
\begin{align*}
...
Let $z$ be a basis for a vector space $V$. For any $\mathbfit{v} \in V$, define $\mathbfit{v}^*_z \coloneqq z^{-1}(\mathbfit{v})$. Clearly $\mathbfit{v}^*_z \in \F^n$. We can view $\mathbfit{v}^*_z$ as the ``coordinates'' of $\mathbfit{v}$ with respect to $z$. Suppose $\alpha \in \yhwidehat{V}$, we define some $\alpha^*_z \in \yhwidehat{\F^n}$ by $\alpha^*_z \coloneqq \alpha \circ z$. In particular, suppose the $i$-th component of $\alpha$ is $\alpha_i$, we notice that
\begin{equation*}
...
\end{dfnbox}
Now looking back at our Definition \ref{dfn:tensorProd}, we consider the vector space $V \otimes \yhwidehat{V}$. For every $\mathbfit{u} \in V$, define $h_{\mathbfit{u}} \colon V \times \yhwidehat{V} \to V$ to be a bilinear map such that $h_{\mathbfit{u}}(\mathbfit{v}, \alpha) = \alpha(\mathbfit{v})\mathbfit{u}$. By Definition \ref{dfn:tensor}, there is a unique map $\widetilde{h}_{\mathbfit{u}} \colon V \otimes \yhwidehat{V} \to V$ such that 
\begin{equation*}
...
\begin{dfnbox}{Tensor Product between Dual Vectors}{dualVecTensor}
    Let $V$ be a finite-dimensional vector space. For any $\alpha, \beta \in \yhwidehat{V}$, their {\color{red} \textbf{tensor product}} is defined as the mapping 
    \begin{equation*}
        \alpha \otimes \beta \colon V \times V \to \F
    \end{equation*}
    such that $(\alpha \otimes \beta)(\mathbfit{u}, \mathbfit{v}) = \alpha(\mathbfit{u})\beta(\mathbfit{v})$.
\end{dfnbox}
We shall also check that this definition stays consistent with the universal property in Definition \ref{dfn:tensor}. For each $\mathbfit{u}, \mathbfit{v} \in V$, define $h_{\mathbfit{u}, \mathbfit{v}} \colon \yhwidehat{V} \times \yhwidehat{V} \to \F$ by $h_{\mathbfit{u}, \mathbfit{v}}(\alpha, \beta) = \alpha(\mathbfit{u})\beta(\mathbfit{v})$. Now, there is a unique map $\widetilde{h}_{\alpha, \beta} \colon \yhwidehat{V} \otimes \yhwidehat{V} \to \F$ such that 
\begin{equation*}
...
\end{equation*}
If we take $\otimes \colon \yhwidehat{V} \times \yhwidehat{V} \to \mathcal{L}\left(V^2, \F\right)$, clearly $\widetilde{h}_{\mathbfit{u}, \mathbfit{v}}(\alpha \otimes \beta) = (\alpha \otimes \beta)(\mathbfit{u}, \mathbfit{v}) = \alpha(\mathbfit{u})\beta(\mathbfit{v})$.

It is easy to see that for any basic vectors $\zeta^i, \zeta^j$ for $\yhwidehat{V}$, the mapping $\zeta^i \otimes \zeta^j$ is a bilinear form over $V$.
\begin{probox}{Basis of the Bilinear Form Space}{bilBasis}
    ...
    \end{proof}
\end{probox}
In this sense, $\mathbb{B}(V) = \mathcal{L}(V^2, \F)$. However, intuitively for each mapping $(\mathbfit{u}, \mathbfit{v}) \mapsto c$, this is equivalent to finding some $T_{\mathbfit{u}} \in \yhwidehat{V}$ for each $\mathbfit{u} \in V$ and apply the mapping to $\mathbfit{v}$.
\begin{probox}{Every Bilinear Maps to a Unique Dual Vector}{BisLVV*}
    For any finite-dimensional vector space $V$, we have $\mathbb{B}(V) \cong \mathcal{L}\left(V, \yhwidehat{V}\right)$.
    \tcblower
    \begin{proof}
        Take any $b \in \mathbb{B}(V)$. Define a mapping $f \colon \mathbb{B}(V) \to  \mathcal{L}\left(V, \yhwidehat{V}\right)$ by $f(b) = b^{\#}$, where $b^{\#}(\mathbfit{v})(\mathbfit{u}) = b(\mathbfit{u}, \mathbfit{v})$. Suppose there are $b_1, b_2 \in \mathbb{B}(V)$ with $b_1 \neq b_2$, then there exists $\mathbfit{u}, \mathbfit{v} \in V$ such that 
        \begin{equation*}
            b_1^{\#}(\mathbfit{v})(\mathbfit{u}) = b_1(\mathbfit{u}, \mathbfit{v}) \neq b_2(\mathbfit{u}, \mathbfit{v}) = b_2^{\#}(\mathbfit{v})(\mathbfit{u}),
        \end{equation*}
        which implies that $b_1^{\#}(\mathbfit{v}) \neq b_2^{\#}(\mathbfit{v})$, and so $b_1^{\#} \neq b_2^{\#}$. Therefore, the $f$ is injective. Now, take any $b^{\#} \in \mathcal{L}\left(V, \yhwidehat{V}\right)$, let $b$ be a bilinear form with $b(\mathbfit{u}, \mathbfit{v}) = b^{\#}(\mathbfit{v})(\mathbfit{u})$, then by definition clearly $f(b) = b^{\#}$. Therefore, $f$ is surjective, and so $\mathbb{B}(V) \cong \mathcal{L}\left(V, \yhwidehat{V}\right)$.
    \end{proof}
\end{probox}
We can write 
\begin{equation*}
...
\end{notebox}
    ...
    Recall that in Proposition \ref{pro:BisLVV*}, we have demonstrated that every bilinear form over a vector space $V$ defines a mapping from $V$ to $\yhwidehat{V}$. This mapping becomes very special if we construct it with respect to an inner product.
    \begin{thmbox}{Riesz Representation Theorem}{Riesz}
        For any inner product space $(V, g)$, let the mapping $\Gamma \colon V \to \yhwidehat{V}$ be such that
        \begin{equation*}
            \Gamma(\mathbfit{u})(\mathbfit{v}) = g(\mathbfit{v}, \mathbfit{u}),
        \end{equation*}
        then for every $\alpha \in \yhwidehat{V}$, there is a unique $\mathbfit{u}_{\alpha} \in V$ such that $\alpha = \Gamma\left(\mathbfit{u}_{\alpha}\right)$.
        \tcblower
        \begin{proof}
            ...
            \begin{equation*}
                \mathrm{rank}(\Gamma) = \dim(V) = \dim\left(\yhwidehat{V}\right),
            \end{equation*}
            and so $\mathrm{range}(\Gamma) = \yhwidehat{V}$, which means that $\Gamma$ is surjective. Therefore, $\Gamma$ is an isomorphism.
        \end{proof}
    \end{thmbox}
    ...
    \begin{probox}{Conversion from Operators to Bilinear Forms}{convertOpToBil}
        ...
        \tcblower
        \begin{proof}
            By Proposition \ref{pro:BisLVV*}, since $\Gamma \circ T \in \mathcal{L}\left(V, \yhwidehat{V}\right)$, there is a unique bilinear form $b_T \colon V \times V \to V$ defined by 
            \begin{equation*}
            ...
            \end{equation*}
            Conversely, for any bilinear form $b \in \mathbb{B}(V)$, we can similarly construct $b^{\#}_T \colon V \to \yhwidehat{V}$ by $b^{\#}_T(\mathbfit{v})(\mathbfit{u}) = b(\mathbfit{u}, \mathbfit{v})$. Therefore, $\Gamma^{-1} \circ b^{\#}_T \in \mathcal{L}(V, V)$ is an operator over $V$. 
        \end{proof}
    \end{probox}
    ...
    \end{dfnbox}
    If we denote the set of all sesquilinear forms over a vector space $V$ as $\mathbb{S}(V)$, then just like Proposition \ref{pro:BisLVV*}, $\mathbb{S}(V) \cong \mathrm{Maps}\left(V, \yhwidehat{V}\right)$. This is because for every $s \in \mathbb{S}(V)$, we can also define some $s^{\#} \in \mathrm{Maps}\left(V, \yhwidehat{V}\right)$ such that $s^{\#}(\mathbfit{v})(\mathbfit{u}) = s(\mathbfit{u}, \mathbfit{v})$. However, notice that now, $s^{\#}$ is no longer linear. Instead, it is said to be \textit{conjugate-linear}.
...
\begin{dfnbox}{Wedge Product}{wedge}
    Let $V$ be a finite-dimensional vector space. For every $\alpha, \beta \in \yhwidehat{V}$, their {\color{red} \textbf{wedge product}} or {\color{red} \textbf{exterior product}} is defined as 
    \begin{equation*}
...
Recall that in Definition \ref{dfn:tranpose}, we defined the transposition of linear transformations. Applying this to $m$-forms, we can similarly define, for each linear operator $T$ on $V$, an operator $\yhwidehat{T}$ on the space of $m$-forms over $V$ by
\begin{equation*}
    \yhwidehat{T}(\Psi)\left(\mathbfit{v}_1, \mathbfit{v}_2, \cdots, \mathbfit{v}_m\right) = \Psi\bigl(T(\mathbfit{v}_1), T(\mathbfit{v}_2), \cdots, T(\mathbfit{v}_m)\bigr).
\end{equation*}
Now, if $V$ happens to be $m$-dimensional, then $\yhwidehat{T}(\Psi)$ has to be some scalar multiple of $\Psi$ because the space of $m$-forms now becomes one-dimensional.
\section{Determinant}
\begin{dfnbox}{Determinant}{determinant}
    ...
    \begin{equation*}
        \yhwidehat{T}(\Omega) = \Delta(T)\Omega
    \end{equation*}
    for every $n$-form $\Omega$ over $V$.
\end{dfnbox}
\begin{notebox}
    \begin{remark}
        Clearly, $\Delta(T)$ is the unique eigenvalue of $\yhwidehat{T}$.
    \end{remark}
\end{notebox}
...
\begin{probox}{Computing Determinant of Operators}{computeDet}
    ...
    \begin{proof}
        ...
        \begin{align*}
            \Delta(T)\Omega & = \yhwidehat{T}(\Omega)\left(\mathbfit{t}_1, \mathbfit{t}_2, \cdots, \mathbfit{t}_n\right) \\
...
\begin{probox}{Operator Composition Preserves Determinant}{detComp}
    ...
    \begin{proof}
        ...
        \begin{align*}
            ...
            & = \yhwidehat{T}(\Omega)\Bigl(S(\mathbfit{v}_1), S(\mathbfit{v}_2), \cdots, S\bigl(\mathbfit{v}_{\dim(V)}\bigr)\Bigr) \\
