...
\begin{dfnbox}{Dual Space}{dualSpace}
    Let $V$ be a vector space over $\mathcal{F}$. The {\color{red} \textbf{dual space}} of $V$ is defined as $\hat{V} \coloneqq \mathcal{L}(V, \mathcal{F})$.
\end{dfnbox}
...
\begin{dfnbox}{Dual Vector}{dualVec}
    Let $V$ be an $n$-dimensional vector space over $\mathcal{F}$. The {\color{red} \textbf{dual vectors}} $\zeta^i \in \hat{V}$ of $V$ with respect to a basis $\left\{\mathbfit{z}_1, \mathbfit{z}_2, \cdots, \mathbfit{z}_n\right\}$ are defined as $\zeta^i(\mathbfit{v}) = a_i$ for each $\mathbfit{v} = \sum_{i = 1}^{n}a_i\mathbfit{z}_i$.
\end{dfnbox}
