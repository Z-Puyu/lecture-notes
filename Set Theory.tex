...
\begin{dfnbox}{Subclass}{subclass}
    Let $\mathbb{A}$ and $\mathbb{B}$ be classes, then $\mathbb{A}$ is a {\color{red} \textbf{subclass}} of $\mathbb{B}$ if for every $X \in \mathbb{A}$, we have $X \in \mathbb{B}$.
\end{dfnbox}
...
\begin{dfnbox}{Universal Class}{univClass}
    ...
    \begin{displaymath}
        \mathbb{V} \coloneqq \left\{X \colon X = X\right\}.
    \end{displaymath}
\end{dfnbox}
\begin{notebox}
    \begin{remark}
...
The following sections discuss each of the axioms in Zermelo-Fraenkel set theory. These seemingly innocent axioms are used to answer the important question: \textbf{what is a set} (and what is not a set)?
\section{Extensionality}
\begin{axibox}{Axiom of Extensionality}{extensionAxi}
...
\begin{thmbox}{The Empty Set}{emptySet}
    ...
    \tcblower   
    \begin{proof}
        Let $X$ be a set with no elements. Notice that this implies that for all $u$, we have $u \notin X$. Let~$Y$ be another set. Note that the statement $u \in X \implies u \in Y$ is vacuously true. Suppose that $Y$ has no elements, then similarly for all $u$, the statement~$u \in Y \implies u \in X$ is also vacuously true. Therefore, for all $u$, we have proven that $u \in X$ if and only if $u \in Y$. By Axiom \ref{axi:extensionAxi}, this means that $X = Y$, i.e. the set with no elements is unique.
    \end{proof}
\end{thmbox}
This set with no elements is known as the \textit{empty set}, denoted by $\varnothing$.
\section{Pairing}
\begin{axibox}{Axiom of Pairing}{pairAxi}
...
\begin{notebox}
    \begin{remark}
        In particular, we can define the \textbf{singleton} $\{a\}$ to be the pair $\{a, a\}$.
    \end{remark}
\end{notebox}
...
\begin{thmbox}{Uniqueness of Pairs}{uniquePairs}
    For all $a$ and $b$, the pair $\{a, b\}$ is unique.
    \tcblower   
    \begin{proof}
...
\begin{dfnbox}{Ordered $n$-tuple}{nTuple}
    ...
    \end{displaymath}
\end{dfnbox}
By Extensionality, we can similarly prove that any two ordered $n$-tuples $(a_1, a_2, \cdots, a_n)$ and~$(b_1, b_2, \cdots, b_n)$ are equal if and only if $a_i = b_i$ for $i = 1, 2, \cdots, n$.
\section{Separtion}
...
\begin{quote}
    Let $\mathbb{C} = \left\{u \colon \varphi(u, \mathbfit{p})\right\}$ be a class, then for any set $X$, there exists some set $Y$ such that $\mathbb{C} \cap X = Y$.
\end{quote}
...
\end{displaymath}
Given some formula $P\left(x, p\right)$, it is possible that in a set $X$, there is no element such that~$P\left(x, p\right)$ is true. In this case, we say that the set induced by $P$ is \textit{disjoint} from $X$.
\begin{dfnbox}{Disjoint}{disjoint}
    Two sets $X$ and $Y$ are called {\color{red} \textbf{disjoint}} if $X \cap Y = \varnothing$.
\end{dfnbox}
Next, intersection can be executed repeatedly over any collection of sets. 
...
\begin{notebox}
    \begin{remark}
        We often call $\bigcup X$ the \textit{unary union} of $X$.
    \end{remark}
\end{notebox}
...
\begin{notebox}
    \begin{remark}
        Note that $X \times Y$ is a set because $X \times Y \subseteq \mathcal{P}\bigl(\mathcal{P}(X \cup Y)\bigr)$.
    \end{remark}
\end{notebox}
The above offers a new way to define $n$-tuples, as we can define Cartesian products of countably many sets recursively.
\begin{dfnbox}{Countable Cartesian Product}{countableProd}
...
\begin{dfnbox}{Equivalence Relation}{equiv}
    ...
    \begin{equation*}
        X\bign\slash\equiv \coloneqq \bigl\{[x] \colon x \in X\bigr\}.
    \end{equation*}
\end{dfnbox}
...
\begin{dfnbox}{Restriction}{Restriction}
    ...
    \end{displaymath}
\end{dfnbox}
The reverse of restriction is called \textit{extension}:
\begin{dfnbox}{Extension}{extension}
    ...
    \end{displaymath}
\end{dfnbox}
Furthermore, we can chain up functions as a pipeline:
...
\begin{thmbox}{Countering Russell's Paradox}{Russell}
...
\begin{axibox}{Axiom of Choice}{axiChoice}
    For every $X$ with $\varnothing \notin X$, there exists a choice function
    \begin{displaymath}
